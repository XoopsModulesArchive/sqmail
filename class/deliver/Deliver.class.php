 ...
 * a delivery backend.
 *
 * $Id: Deliver.class.php,v 1.18 2003/03/07 22:12:06 stekkel Exp $
 */
class Deliver
{
    public function mail($message, $stream = false)
    {
        ...
        $reply_rfc822_header = ($message->reply_rfc822_header ?? '');
    ...
    public function writeBody($message, $stream, &$length_raw, $boundary = '')
    {
        ...
        $boundary_depth = mb_substr_count($message->entity_id, '.');
    ...
    public function writeBodyPart($message, $stream, &$length)
    {
            ...
            default:
                if ($message->body_part) {
                    ...
                    while ($tmp = fread($file, 570)) {
                        $body_part = chunk_preg_split(base64_encode($tmp));
        ...
        if ($body_part_trailing) {
            $length += mb_strlen($body_part_trailing);
    ...
    public function writeToStream($stream, $data)
    {
        fwrite($stream, $data);
    }

    public function initStream($message, $length = 0, $host = '', $port = '', $user = '', $pass = '')
    {
        return $stream;
    }
    ...
    public function prepareMIME_Header($message, $boundary)
    {
        ...
        if (isset($mime_header->parameters['name'])) {
            $contenttype .= '; name="' . encodeHeader($mime_header->parameters['name']) . '"';
        }

        if (isset($mime_header->parameters['charset'])) {
            $charset = $mime_header->parameters['charset'];

            $contenttype .= '; charset="' . encodeHeader($charset) . '"';
        }
        ...
        if ($mime_header->encoding) {
            ...
            if ('text' == $mime_header->type0 || 'message' == $mime_header->type0) {
        ...
        if ($mime_header->disposition) {
            ...
            if ($disposition->getProperty('filename')) {
                $contentdisp .= '; filename="' . encodeHeader($disposition->getProperty('filename')) . '"';
            }
    ...
    public function prepareRFC822_Header($rfc822_header, $reply_rfc822_header, &$raw_length)
    {
    ...
    */
    ...
    public function foldLine($line, $length, $pre = '')
    {
        ...
        if ($cnt > $length) { /* try folding */
            ...
            while (mb_strlen($line) > $length) {
                ...
                if (preg_match('/(=\?([^?]*)\?(Q|B)\?([^?]*)\?=)(.*)/Ui', $line, $regs)) {
                    ...
                    $iPosEnc = mb_strpos($line, $fold_tmp);

                    $iLengthEnc = mb_strlen($fold_tmp);
                    ...
                    if ($iPosEnc < $length && (($iPosEncEnd) > $length)) {
                        ...
                        $aFoldLine[] = mb_substr($line, 0, $iPosEnc);

                        $line = mb_substr($line, $iPosEnc);

                        if (!$bFirstFold) {
                            $bFirstFold = true;

                            $length -= mb_strlen($fold_string);
                        }
                        ...
                        if ($iLengthEnc > $length) { /* place the encoded
                            ...
                            $aFoldLine[] = mb_substr($line, 0, $iLengthEnc);

                            $line = mb_substr($line, $iLengthEnc);
                        }
                    } elseif ($iPosEnc < $length) { /* the encoded string fits into the foldlength */
                        /*remainder */

                        $sLineRem = mb_substr($line, $iPosEncEnd, $length - $iPosEncEnd);

                        if (preg_match('/^(=\?([^?]*)\?(Q|B)\?([^?]*)\?=)(.*)/Ui', $sLineRem) || !preg_match('/[=,;\s]/', $sLineRem)) {
                            /*impossible to fold clean in the next part -> fold after the enc string */

                            $aFoldLine[] = mb_substr($line, 0, $iPosEncEnd + 1);

                            $line = mb_substr($line, $iPosEncEnd + 1);
                ...
                if (!$fold) {
                    $line_tmp = mb_substr($line, 0, $length);
                    ...
                    switch (true) {
                        case ($iFoldPos = mb_strrpos($line_tmp, ',')):
                            break;
                        case ($iFoldPos = mb_strrpos($line_tmp, ';')):
                            break;
                        case ($iFoldPos = mb_strrpos($line_tmp, ' ')):
                    ...
                    $aFoldLine[] = mb_substr($line, 0, $iFoldPos + 1);

                    $line = mb_substr($line, $iFoldPos + 1);
    ...
    public function mimeBoundary()
    {
        static $mimeBoundaryString;

        if (!isset($mimeBoundaryString)
            || '' == $mimeBoundaryString) {
            $mimeBoundaryString = '----=_' . date('YmdHis') . '_' . mt_rand(10000, 99999);
        }
    ...
    public function calculate_references($hdr)
    {
