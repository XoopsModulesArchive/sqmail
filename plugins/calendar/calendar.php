 ...
 *
 * $Id: calendar.php,v 1.13 2002/12/31 12:49:34 kink Exp $
 */
define('SM_PATH', '../../');
...
function startcalendar()
{
    ...
    echo html_tag(
        ...
        "\n" . html_tag(
                 'td',
                 "\n" . html_tag('table', '', '', $color[0], 'width="100%" border="0" cellpadding="2" cellspacing="1"') . html_tag(
                     'tr',
                     "\n" . html_tag(
                         'th',
                         ...
                         'th',
                         "<a href=\"$self?year=$prev_year&amp;month=$prev_month\">&lt;&nbsp;" . date_intl('M', $prev_date) . '</a>'
                     ) . "\n" . html_tag('th', date_intl('F Y', $act_date), '', $color[0], 'colspan="3"') . html_tag(
                         'th',
                         "<a href=\"$self?year=$next_year&amp;month=$next_month\">" . date_intl('M', $next_date) . '&nbsp;&gt;</a>'
                     ) . "\n" . html_tag(
                         'th',
                         "<a href=\"$self?year=" . ($year + 1) . "&amp;month=$month\">" . ($year + 1) . '&nbsp;&gt;&gt;</a>'
                     )
...
function drawmonthview()
{
    ...
    while ($aday <= $days_in_month) {
        ...
        for ($j = 1; $j <= 7; $j++) {
            ...
            ($aday < 10) ? $cdate .= "0$aday" : $cdate .= (string)$aday;
            ...
            if (isset($calendardata[$cdate])) {
                ...
                while ($calfoo = each($calendardata[$cdate])) {
                    ...
                    echo (1 == $calbar['priority']) ? "<a href=\"#\" style=\"text-decoration:none; color: $color[1]\" title=\"$calbar[message]\">$calbar[title]</a><br>\n" : "<a href=\"#\" style=\"text-decoration:none; color: $color[6]\" title=\"$calbar[message]\">$calbar[title]</a><br>\n";
