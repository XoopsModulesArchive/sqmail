 ...
 *
 * $Id: event_edit.php,v 1.14 2003/01/11 12:49:21 kink Exp $
 */
define('SM_PATH', '../../');
...
function show_event_form()
{
    ...
    echo "      </SELECT>\n"
         ...
         . html_tag(
             'td',
             '<INPUT TYPE=SUBMIT NAME=send VALUE="' . _('Update Event') . "\">\n",
             'left',
             $color[4],
             'colspan="2"'
         )
...
echo html_tag('tr', '', '', $color[0]) . html_tag('td', '', 'left') . html_tag('table', '', '', $color[0], 'width="100%" border="0" cellpadding="2" cellspacing="1"') . html_tag('tr') . html_tag(
    'td',
...
if (!isset($updated)) {
    ...
    if (!isset($confirmed)) {
        ...
        $event_text = preg_replace("\r", '', $event_text);
        ...
        if ($year == $event_year) {
            $calendardata[(string)$fixdate]["$event_hour$event_minute"] = ['length' => (string)$event_length, 'priority' => (string)$event_priority, 'title' => (string)$event_title, 'message' => (string)$event_text];
