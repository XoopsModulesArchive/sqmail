 ...
 *
 * $Id: setup.php,v 1.14 2003/03/11 15:57:20 kink Exp $
 */
...
{
    /* Standard initialization API. */ global $squirrelmail_plugin_hooks;

    /* The hooks to make the sent subfolders display correctly. */

    $squirrelmail_plugin_hooks['check_handleAsSent_result']['sent_subfolders'] = 'sent_subfolders_check_handleAsSent';

    /* The hooks to automatically update sent subfolders. */

    $squirrelmail_plugin_hooks['left_main_before']['sent_subfolders'] = 'sent_subfolders_update_sentfolder';

    $squirrelmail_plugin_hooks['compose_send']['sent_subfolders'] = 'sent_subfolders_update_sentfolder';

    /* The hook to load the sent subfolders prefs. */

    $squirrelmail_plugin_hooks['loading_prefs']['sent_subfolders'] = 'sent_subfolders_load_prefs';

    /* The hooks to handle sent subfolders options. */

    $squirrelmail_plugin_hooks['optpage_loadhook_folder']['sent_subfolders'] = 'sent_subfolders_optpage_loadhook_folders';
}

function sent_subfolders_check_handleAsSent()
{
    global $handleAsSent_result, $sent_subfolders_base, $use_sent_subfolders;
    ...
    if ($use_sent_subfolders && (count($args) > 1)) {
        ...
        $mbox_str = mb_substr($args[1], 0, mb_strlen($base_str));

        /* Perform the comparison. */

        $handleAsSent_result = ($handleAsSent_result
                                || ($base_str == $mbox_str)
...
{
    global $use_sent_subfolders, $data_dir, $username, $sent_subfolders_setting, $sent_subfolders_base;
    ...
    $sent_subfolders_base = getPref($data_dir, $username, 'sent_subfolders_base', SMPREF_NONE);
}
...
function sent_subfolders_optpage_loadhook_folders()
{
    ...
    $optvals[] = [
        ...
        'posvals' => [
            SMPREF_SENT_SUBFOLDERS_DISABLED => _('Disabled'),
    ...
    foreach ($boxes as $folder) {
        if ('inbox' != mb_strtolower($folder['unformatted'])) {
...
function save_option_sent_subfolders_setting($option)
{
    ...
    if (SMPREF_SENT_SUBFOLDERS_DISABLED == $option->new_value) {
        ...
        if ($sent_subfolder != $sent_folder) {
            ...
            if ('none' != $sent_folder) {
                ...
                if (!sqimap_mailbox_exists($ic, $year_folder)) {
                    sqimap_mailbox_create($ic, $year_folder, (1 == $level) ? '' : 'noselect');
                } elseif (!sqimap_mailbox_is_subscribed($ic, $year_folder)) {
                ...
                } elseif (!sqimap_mailbox_is_subscribed($ic, $sent_subfolder)) {
