 ...
 *
 * $Id: take.php,v 1.8 2003/03/27 16:47:44 philippe_mingo Exp $
 */
    ...
    'width="100%" cols="1"'
     ...
     html_tag('table', '', 'center', '', 'border="0" cellpadding="1" cols="2" width="90%"') . "\n" . html_tag(
         ...
         "\n" . html_tag('td', _('Nickname') . ':', 'right', $color[4], 'width="50"') . "\n" . html_tag(
             'td',
             '<input name="' . $name . '[nickname]" size="15" value="">' . '&nbsp;<small>' . _('Must be unique') . '</small>',
             'left',
             $color[4]
         )
...
foreach ($email as $Val) {
    if (valid_email($Val, $abook_take_verify)) {
        echo '<option value="' . htmlspecialchars($Val, ENT_QUOTES | ENT_HTML5) . '">' . htmlspecialchars($Val, ENT_QUOTES | ENT_HTML5) . "</option>\n";
    } else {
        echo '<option value="' . htmlspecialchars($Val, ENT_QUOTES | ENT_HTML5) . '">FAIL - ' . htmlspecialchars($Val, ENT_QUOTES | ENT_HTML5) . "</option>\n";
    }
}
