...
function valid_email($email, $verify)
{
    ...
    return checkdnsrr(mb_substr(mb_strstr($email, '@'), 1), 'ANY');
}
...
function abook_take_read_string($str)
{
    ...
    while (eregi('(' . $Email_RegExp_Match . ')', $str, $hits)) {
        $str = mb_substr(mb_strstr($str, $hits[0]), mb_strlen($hits[0]));

        if (!isset($abook_found_email[$hits[0]])) {
            echo '<input type="hidden" name="email[]" value="' . htmlspecialchars($hits[0], ENT_QUOTES | ENT_HTML5) . "\">\n";
...
function abook_take_read()
{
    ...
    echo '<input type="submit" value="' . _('Take Address') . '">' . '</center></form>';
}
