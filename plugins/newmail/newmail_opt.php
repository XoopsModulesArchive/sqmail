 ...
 *
 * $Id: newmail_opt.php,v 1.17 2003/03/31 04:18:20 ebullient Exp $
 */
define('SM_PATH', '../../');
...
echo '<form action="' . sqm_baseuri() . 'src/options.php" method=post>' . html_tag('table', '', '', '', 'width="100%" cellpadding="0" cellspacing="2" border="0"') . "\n" . html_tag('tr') . "\n" . html_tag('td', '&nbsp;', 'right', '', 'nowrap') . "\n";
if ('true' == $allowsound) {
    echo html_tag('td', '', 'left') . '<input type="checkbox" ';

    if ('on' == $media_enable) {
        echo 'checked ';
    }

    echo 'name="media_enable"><b> ' . _('Enable Media Playing') . '</b></td>' . '</tr>' . "\n";
}
echo html_tag('tr') . "\n" . html_tag('td', '&nbsp;', 'right', '', 'nowrap') . "\n" . html_tag('td', '', 'left') . '<input type="checkbox" ';
if ('on' == $media_allbox) {
    echo 'checked ';
}
echo 'name="media_allbox"><b> ' . _('Check all boxes, not just INBOX') . '</b></td>' . '</tr>' . "\n" . html_tag('tr') . "\n" . html_tag('td', '&nbsp;', 'right', '', 'nowrap') . "\n" . html_tag('td', '', 'left') . '<input type="checkbox" ';
if ('on' == $media_recent) {
    echo 'checked ';
}
echo 'name="media_recent"><b> ' . _('Count only messages that are RECENT') . '</b></td>' . '</tr>' . "\n" . html_tag('tr') . "\n" . html_tag('td', '&nbsp;', 'right', '', 'nowrap') . "\n" . html_tag('td', '', 'left') . '<input type="checkbox" ';
if ('on' == $media_changetitle) {
    echo 'checked ';
}
echo 'name="media_changetitle"><b> ' . _('Change title on supported browsers.') . '</b> &nbsp; (' . _('requires JavaScript to work') . ')</td>' . '</tr>' . "\n" . html_tag('tr') . "\n" . html_tag('td', '&nbsp;', 'right', '', 'nowrap') . "\n" . html_tag('td', '', 'left') . '<input type="checkbox" ';
if ('on' == $media_popup) {
    echo 'checked ';
}
echo 'name="media_popup"><b> ' . _('Show popup window on new mail') . '</b> &nbsp; (' . _('requires JavaScript to work') . ')</td>' . '</tr>' . "\n";
if ('true' == $allowsound) {
    echo html_tag('tr') . "\n" . html_tag('td', _('Select server file:'), 'right', '', 'nowrap') . "\n" . html_tag('td', '', 'left') . '<select name="media_sel">' . "\n" . '<option value="(none)"';
    ...
    echo '</select>' . '<input type="submit" value=" ' . _('Try') . ' " name="test" onClick="' . "window.open('testsound.php?sound='+media_sel.options[media_sel.selectedIndex].value, 'TestSound'," . "'width=150,height=30,scrollbars=no');" . 'return false;' . '">' . '</td>' . '</tr>' . html_tag(
        'tr',
        "\n" . html_tag('td', _('Current File:'), 'right', '', 'nowrap') . html_tag('td', '<input type="hidden" value="' . $media . '" name="media_default">' . $media . '', 'left')
    ) . "\n";
}
echo html_tag(
    'tr',
    "\n" . html_tag('td', '&nbsp;') . html_tag(
             'td',
             '<input type="hidden" name="optmode" value="submit">' . '<input type="submit" value="' . _('Submit') . '" name="submit_newmail">',
             'left'
         )
) . "\n" . '</table>' . "\n" . '</form>' . "\n" . '</td></tr></table>' . "\n" . '</body></html>';
