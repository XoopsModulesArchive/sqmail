 ...
 * too (with a plugin).
 *
 * $Id: setup.php,v 1.17 2003/03/31 04:18:20 ebullient Exp $
 */
...
function newmail_sav()
{
    ...
    if (sqgetGlobalVar('submit_newmail', $submit, SQ_POST)) {
        ...
        if (sqgetGlobalVar('media_sel', $media_sel, SQ_POST)
            && ('(none)' == $media_sel || '(local media)' == $media_sel)) {
...
{
    global $username, $key, $imapServerAddress, $imapPort, $newmail_media, $newmail_enable, $newmail_popup, $newmail_recent, $newmail_changetitle, $imapConnection, $PHP_SELF;

    if ('on' == $newmail_enable
        || 'on' == $newmail_popup
        || $newmail_changetitle) {
        ...
        for ($i = 0, $iMax = count($boxes); $i < $iMax; $i++) {
            ...
            if ($boxes[$i]['flags']) {
                ...
                for ($h = 0, $hMax = count($boxes[$i]['flags']); $h < $hMax; $h++) {
                    if ('noselect' == mb_strtolower($boxes[$i]['flags'][$h])) {
                        $noselect = true;
                    }
                }

                if (!$noselect) {
                    $status += CheckNewMailboxSound(
                        $imapConnection,
                        $mailbox,
                        $boxes[$i]['unformatted'],
                        $delimeter,
        ...
        if ($totalNew > 0 && 'on' == $newmail_enable && '' != $newmail_media) {
            echo "<EMBED SRC=\"$newmail_media\" HIDDEN=TRUE AUTOSTART=TRUE>\n";
        }

        if ($totalNew > 0 && 'on' == $newmail_popup) {
            echo "<SCRIPT LANGUAGE=\"JavaScript\">\n"
                 . "<!--\n"
                 . "function PopupScriptLoad() {\n"
                 . 'window.open("'
                 . sqm_baseuri()
                 . 'plugins/newmail/newmail.php", "SMPopup",'
