 ...
 *
 * $Id: setup.php,v 1.11 2003/02/02 09:36:24 tokul Exp $
 */
...
function translate_read_form()
{
    ...
    $trans_ar = $message->findDisplayEntity([], ['text/plain']);

    $body = '';

    if ('' != $trans_ar[0]) {
        for ($i = 0, $iMax = count($trans_ar); $i < $iMax; $i++) {
    ...
    if (is_int($pos)) {
        $new_body = mb_substr($new_body, 0, $pos);
    }
...
function translate_does_it_match_language($test)
{
    ...
    if ((0 == $index) && ($test == $squirrelmail_language)) {
        return $true;
    }
...
function translate_lang_opt($from, $to, $value, $text)
{
    ...
    if (translate_does_it_match_language($to) && ('to' == $translate_dir)) {
        $ret .= ' SELECTED';
    }

    if (translate_does_it_match_language($from) && ('from' == $translate_dir)) {
        $ret .= ' SELECTED';
    }
                ...
                <tr>
                    <td><?php
                        ...
                        <select name="lp"><?php
                            ...
                            echo '</select>' . 'Babelfish: <input type="Submit" value="' . _('Translate') . '">';
                            ...
                            <select name=lp><?php
                                ...
                                echo '</select>' . "<input type=\"hidden\" name=\"text\" value=\"$message\">" . 'Go.com: <input type="Submit" value="' . _('Translate') . '">';
                                ...
                                function translate_form_intertran($message)
                                {
                                    ...
                                    echo '<INPUT TYPE="hidden" NAME="topframe" VALUE="yes">' . '<INPUT TYPE="hidden" NAME="type" VALUE="text">' . "<input type=\"hidden\" name=\"text\" value=\"$message\">";
                                ...
                                function translate_form_gpltrans($message)
                                {
                                    ...
                                    echo "<input type=hidden name=text value=\"$message\">" . 'GPLTrans: <input type="submit" value="' . _('Translate') . '">';
                                ...
                                function translate_form_dictionary($message)
                                {
                                    ...
                                    echo "<input type=hidden name=text value=\"$message\">" . "<input type=hidden name=r value=\"$time\">" . '<SELECT NAME="lp">' . translate_lang_opt(
                                        'en',
                                        'fr',
                                        'en_fr',
                                        sprintf(
                                         ...
                                         ) . '</SELECT>' . 'Dictionary.com: <INPUT TYPE="submit" VALUE="' . _('Translate') . '">';
