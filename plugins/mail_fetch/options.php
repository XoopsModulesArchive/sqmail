 ...
 *
 * $Id: options.php,v 1.21 2003/03/26 16:30:01 kink Exp $
 */
define('SM_PATH', '../../');
...
switch ($mf_action) {
    case 'add':
        ...
        setPref($data_dir, $username, "mailfetch_server_$mf_sn", ($mf_server ?? ''));
        setPref($data_dir, $username, "mailfetch_port_$mf_sn", ($mf_port ?? 110));
        setPref($data_dir, $username, "mailfetch_alias_$mf_sn", ($mf_alias ?? ''));
        setPref($data_dir, $username, "mailfetch_user_$mf_sn", ($mf_user ?? ''));
        setPref($data_dir, $username, "mailfetch_pass_$mf_sn", (isset($mf_pass) ? encrypt($mf_pass) : ''));
        if (isset($mf_cypher) && 'on' != $mf_cypher) {
            SetPref($data_dir, $username, 'mailfetch_cypher', 'on');
        }
        setPref($data_dir, $username, "mailfetch_lmos_$mf_sn", ($mf_lmos ?? ''));
        setPref($data_dir, $username, "mailfetch_login_$mf_sn", ($mf_login ?? ''));
        setPref($data_dir, $username, "mailfetch_fref_$mf_sn", ($mf_fref ?? ''));
        setPref($data_dir, $username, "mailfetch_subfolder_$mf_sn", ($mf_subfolder ?? ''));
        $mf_sn++;
...
echo '<br><form method="post" action="' . $PHP_SELF . '">' . html_tag(
    'table',
    html_tag(
            'tr',
            html_tag(
    ...
    'width="95%" cols="1"'
        ...
        html_tag(
             ...
             html_tag(
                 'td',
                 '<input type=checkbox name=mf_cypher ' . (('on' == $mailfetch_cypher) ? 'checked >' : '>') . _('Encrypt passwords (informative only)'),
                 'right'
             )
         ),
        'center',
...
switch ($mf_action) {
    case 'config':
        echo html_tag('table', '', 'center', '', 'width="70%" cols="1" cellpadding="5" cellspacing="1"') . html_tag(
            'tr',
            html_tag('td', '<b>' . _('Add Server') . '</b>', 'center', $color[9])
        ) . html_tag('tr') . html_tag('td', '', 'center', $color[0]) .

             "<INPUT TYPE=\"hidden\" NAME=\"mf_sn\" VALUE=\"$mailfetch_server_number\">" . '<INPUT TYPE="hidden" NAME="mf_action" VALUE="add">' . html_tag('table') . html_tag(
                 'tr',
                 html_tag('th', _('Server:'), 'right') . html_tag('td', '<input type=text name=mf_server value="" size=40>', 'left')
             ) . html_tag(
                 'tr',
                 html_tag('th', _('Port:'), 'right') . html_tag('td', '<input type=text name=mf_port value="110" size=20', 'left')
             ) . html_tag(
                 'tr',
                 html_tag('th', _('Alias:'), 'right') . html_tag('td', '<input type=text name=mf_alias value="" size=20>', 'left')
             ) . html_tag(
                 'tr',
                 html_tag('th', _('Username:'), 'right') . html_tag('td', '<input type=text name=mf_user value="" size=20>', 'left')
             ) . html_tag(
                 'tr',
                 html_tag('th', _('Password:'), 'right') . html_tag('td', '<input type=password name=mf_pass value="" size=20>', 'left')
             ) . html_tag('tr') . html_tag('th', _('Store in Folder:'), 'right') . html_tag('td', '', 'left');
        ...
        if (isset($mf_subfolder)) {
            $selected = [mb_strtolower($mf_subfolder)];
        }
        echo sqimap_mailbox_option_list($imapConnection, $selected);
        echo '</select></td></tr>' . html_tag(
            'tr',
            html_tag('th', '&nbsp;', 'right') . html_tag('td', '<input type="checkbox" name="mf_lmos" checked>' . _('Leave Mail on Server'), 'left')
        ) . html_tag(
                'tr',
                html_tag('th', '&nbsp;', 'right') . html_tag('td', '<input type="checkbox" name="mf_login">' . _('Check mail during login'), 'left')
            ) . html_tag(
                 'tr',
                 html_tag('th', '&nbsp;', 'right') . html_tag('td', '<input type="checkbox" name="mf_fref">' . _('Check mail during folder refresh'), 'left')
             ) . html_tag(
                 'tr',
                 html_tag(
                     'td',
        ...
        // Modify Server
        echo '<font size=-5><BR></font>' . html_tag('table', '', 'center', '', 'width="70%" cols="1" cellpadding="5" cellspacing="1"') . html_tag(
            'tr',
        ...
        if ($mailfetch_server_number > 0) {
            ...
            for ($i = 0; $i < $mailfetch_server_number; $i++) {
                echo "<option value=\"$i\">" . htmlspecialchars((('' == $mailfetch_alias_[$i]) ? $mailfetch_server_[$i] : $mailfetch_alias_[$i]), ENT_QUOTES | ENT_HTML5) . '</option>>';
            }

            echo '</select>' . '&nbsp;&nbsp;<INPUT TYPE=submit name=mf_action value="' . _('Modify') . '">' . '&nbsp;&nbsp;<INPUT TYPE=submit name=mf_action value="' . _('Delete') . '">' . '</form>';
        } else {
    ...
    case _('Delete'):                                     //erase confirmation about a server
        echo html_tag(
            ...
            'center',
            '',
            'width="95%" cols="1" cellpadding="5" cellspacing="1"'
        ) . '<br>' . html_tag(
                 'table',
                 html_tag(
                     'tr',
                     ...
                     html_tag(
                         ...
                         . _('Selected Server:')
                         . ' <b>'
                         . htmlentities($mailfetch_server_[$mf_sn], ENT_QUOTES | ENT_HTML5)
                         . '</b><br>'
    ...
    case _('Modify'):
        echo html_tag(
            ...
            '',
            'width="95%" cols="1" cellpadding="5" cellspacing="1"'
        ) . '<br>' . html_tag('table', '', 'center', '', 'width="70%" cols="1" cellpadding="5" cellspacing="1"') . html_tag(
                 'tr',
                 html_tag('td', '<b>' . _('Mofify a Server') . '</b>', 'center', $color[9])
             ) . html_tag('tr') . html_tag('td', '', 'center', $color[0]) .

             "<INPUT TYPE=\"hidden\" NAME=\"mf_sn\" VALUE=\"$mf_sn\">" . '<INPUT TYPE="hidden" NAME="mf_action" VALUE="confirm_modify">' . html_tag('table') . html_tag(
                 'tr',
                 html_tag('th', _('Server:'), 'right') . html_tag('td', '<input type="text" name="mf_server" value="' . htmlentities($mailfetch_server_[$mf_sn], ENT_QUOTES | ENT_HTML5) . '" size="40">', 'left')
             ) . html_tag(
                 'tr',
                 html_tag('th', _('Port:'), 'right') . html_tag('td', '<input type="text" name="mf_port" value="' . $mailfetch_port_[$mf_sn] . '" size="40">', 'left')
             ) . html_tag(
                 'tr',
                 html_tag('th', _('Alias:'), 'right') . html_tag('td', '<input type="text" name="mf_alias" value="' . htmlentities($mailfetch_alias_[$mf_sn], ENT_QUOTES | ENT_HTML5) . '" size="40">', 'left')
             ) . html_tag(
                 'tr',
                 html_tag('th', _('Username:'), 'right') . html_tag('td', '<input type="text" name="mf_user" value="' . $mailfetch_user_[$mf_sn] . '" size="20">', 'left')
             ) . html_tag(
                 'tr',
                 html_tag('th', _('Password:'), 'right') . html_tag('td', '<input type="password" name="mf_pass" value="' . $mailfetch_pass_[$mf_sn] . '" size="20">', 'left')
             ) . html_tag('tr') . html_tag('th', _('Store in Folder:'), 'right') . html_tag('td', '', 'left');
        ...
        if (isset($mailfetch_subfolder_[$mf_sn])) {
            $selected = [mb_strtolower($mailfetch_subfolder_[$mf_sn])];
        }
        echo sqimap_mailbox_option_list($imapConnection, $selected) . '</select></td></tr>' .

             html_tag(
                 'tr',
                 html_tag('th', '&nbsp;', 'right') . html_tag(
                     'td',
                     '<input type=checkbox name=mf_lmos ' . (('on' == $mailfetch_lmos_[$mf_sn]) ? 'checked' : '') . '>' . _('Leave Mail on Server'),
                     'left'
                 )
             ) . html_tag(
                 'tr',
                 html_tag('th', '&nbsp;', 'right') . html_tag(
                     'td',
                     '<input type=checkbox name=mf_login ' . (('on' == $mailfetch_login_[$mf_sn]) ? 'checked' : '') . '>' . _('Check mail during login'),
                     'left'
                 )
             ) . html_tag(
                 'tr',
                 html_tag('th', '&nbsp;', 'right') . html_tag(
                     'td',
                     '<input type=checkbox name=mf_fref ' . (('on' == $mailfetch_fref_[$mf_sn]) ? 'checked' : '') . '>' . _('Check mail during folder refresh'),
                     'left'
                 )
             ) . html_tag(
                 'tr',
    ...
    default:                                    //unsupported action
        echo '</form>' . html_tag(
            ...
            'width="95%" cols="1"'
                ...
                html_tag(
                     'tr',
                     html_tag('td', '<b>' . _('Undefined Function') . '</b>', 'center', $color[9]) . html_tag('td', '<b>' . _('Hey! Wath do You are looking for?') . '</b>', 'center', $color[0])
                 ),
                'center',
                '',
                'width="70%" cols="1"'
