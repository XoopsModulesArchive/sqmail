 ...
 * Fetch code.
 *
 * $Id: fetch.php,v 1.19 2003/03/11 16:57:21 kink Exp $
 */
define('SM_PATH', '../../');
...
function Mail_Fetch_Status($msg)
{
    echo html_tag(
        ...
        html_tag(
            'tr',
            html_tag('td', htmlspecialchars($msg, ENT_QUOTES | ENT_HTML5), 'left')
        ),
        '',
        '',
        'width="90%"'
...
if (!isset($server_to_fetch)) {
    ...
    for ($i = 0; $i < $mailfetch_server_number; $i++) {
        if ('' == $mailfetch_pass_[$i]) {
            echo html_tag(
                ...
                html_tag(
                    'td',
                    _('Password for') . ' <b>' . htmlspecialchars((('' == $mailfetch_alias_[$i]) ? $mailfetch_server_[$i] : $mailfetch_alias_[$i]), ENT_QUOTES | ENT_HTML5) . '</b>: &nbsp; &nbsp; ',
                    'right'
    ...
    echo html_tag(
        'tr',
        html_tag('td', '&nbsp;') . html_tag('td', '<input type=submit name=submit_mailfetch value="' . _('Fetch Mail') . '">', 'left')
    ) . '</table></form>';

    exit();
}

if ('all' == $server_to_fetch) {
    $i_start = 0;
...
for ($i_loop = $i_start; $i_loop < $i_stop; $i_loop++) {
    ...
    echo '<br>' . html_tag(
        ...
        html_tag(
                ...
                html_tag(
                    'td',
                    '<b>' . _('Fetching from ') . htmlspecialchars((('' == $mailfetch_alias_[$i_loop]) ? $mailfetch_server : $mailfetch_alias_[$i_loop]), ENT_QUOTES | ENT_HTML5) . '</b>',
                    'center'
                ),
                '',
                $color[9]
    ...
    for (; $i <= $Count; $i++) {
        Mail_Fetch_Status(_('Fetching message ') . (string)$i);
        ...
        while ((!$MessArray) or ('array' != gettype($MessArray))) {
            Mail_Fetch_Status(_('Oops, ') . $pop3->ERROR);
            ...
            Mail_Fetch_Status(_('Refetching message ') . (string)$i);
