 ...
 *
 * $Id: options.php,v 1.26 2003/03/11 16:57:20 kink Exp $
 */
...
if (isset($_POST['filter_submit'])) {
    ...
    if (('Header' == $filter_where) && ('' == strstr($filter_what, ':'))) {
    ...
    remove_filter($theid);
} elseif (isset($action) && 'move_up' == $action) {
    filter_swap($theid, $theid - 1);
} elseif (isset($action) && 'move_down' == $action) {
    filter_swap($theid, $theid + 1);
...
if ('new' == $filters_user_scan) {
    echo ' selected';
}
echo '>' . _('Only unread messages') . '</option>' . '</select>' . '</td>' . html_tag('td', '<input type=submit name="user_submit" value="' . _('Save') . '">', 'left') . '</table>' . '</center>' . '</form>' .

     html_tag(
         'div',
         '[<a href="options.php?action=add">' . _('New') . '</a>] - [<a href="' . SM_PATH . 'src/options.php">' . _('Done') . '</a>]',
         'center'
     ) . '<br>';

if (isset($action) && ('add' == $action || 'edit' == $action)) {
    ...
    echo html_tag('div', '', 'center') . '<form action="options.php" method=post>' . html_tag('table', '', '', '', 'border="0" cellpadding="2" cellspacing="0"') . html_tag('tr') . html_tag('td', _('Match:'), 'left') . html_tag('td', '', 'left') . '<select name=filter_where>';

    $L = isset($filters[$theid]['where']);

    $sel = (($L && 'From' == $filters[$theid]['where']) ? 'selected' : '');

    echo "<option value=\"From\" $sel>" . _('From') . '</option>';

    $sel = (($L && 'To' == $filters[$theid]['where']) ? 'selected' : '');

    echo "<option value=\"To\" $sel>" . _('To') . '</option>';

    $sel = (($L && 'Cc' == $filters[$theid]['where']) ? 'selected' : '');

    echo "<option value=\"Cc\" $sel>" . _('Cc') . '</option>';

    $sel = (($L && 'To or Cc' == $filters[$theid]['where']) ? 'selected' : '');

    echo "<option value=\"To or Cc\" $sel>" . _('To or Cc') . '</option>';

    $sel = (($L && 'Subject' == $filters[$theid]['where']) ? 'selected' : '');

    echo "<option value=\"Subject\" $sel>" . _('Subject') . '</option>';

    $sel = (($L && 'Header' == $filters[$theid]['where']) ? 'selected' : '');

    echo "<option value=\"Header\" $sel>" . _('Header') . '</option>';

    echo '</select>' . '</td>' . '</tr>' . html_tag('tr') . html_tag('td', _('Contains:'), 'right') . html_tag('td', '', 'left') . '<input type=text size=32 name=filter_what value="';

    if (isset($filters[$theid]['what'])) {
        echo $filters[$theid]['what'];
    }

    echo '">' . '</td>' . '</tr>' . html_tag('tr') . html_tag('td', _('Move to:'), 'left') . html_tag('td', '', 'left') . '<tt>' . '<select name=filter_folder>';

    $selected = 0;

    if (isset($filters[$theid]['folder'])) {
        $selected = [mb_strtolower($filters[$theid]['folder'])];
    }

    echo sqimap_mailbox_option_list(0, $selected, 0, $boxes);

    echo '</tt>' . '</select>' . '</td>' . '</tr>' . '</table>' . '<input type=submit name=filter_submit value=' . _('Submit') . '>' . "<input type=hidden name=theid value=$theid>" . '</form>' . '</div>';
}
...
for ($i = 0, $num = count($filters); $i < $num; $i++) {
    ...
    echo html_tag('tr', '', '', $clr) . html_tag(
        'td',
        '<small>' . "[<a href=\"options.php?theid=$i&action=edit\">" . _('Edit') . '</a>]' . '</small>',
        'left'
    ) . html_tag(
            'td',
            '<small>' . "[<a href=\"options.php?theid=$i&action=delete\">" . _('Delete') . '</a>]' . '</small>',
            'left'
    ...
    echo ']</small></td>' . html_tag('td', '-', 'left') . html_tag('td', '', 'left');
