 ...
 *
 * $Id: spamoptions.php,v 1.15 2003/03/11 16:57:21 kink Exp $
 */
...
if (' ' == $SpamFilters_YourHop) {
    echo '<br>' . html_tag(
        'div',
        '<b>' . _('WARNING! Tell your admin to set the SpamFilters_YourHop variable') . '</b>',
        'center'
    ) . '<br>';
}

if (isset($action) && 'spam' == $action) {
    ...
    for ($i = 0; $i < $numboxes && '' == $filters_spam_folder; $i++) {
        if ((isset($boxes[$i]['flags'][0]) && 'noselect' != $boxes[$i]['flags'][0]) && (isset($boxes[$i]['flags'][1]) && 'noselect' != $boxes[$i]['flags'][1]) && (isset($boxes[$i]['flags'][2]) && 'noselect' != $boxes[$i]['flags'][2])) {
    ...
    if (isset($filters_spam_folder)) {
        $selected = [mb_strtolower($filters_spam_folder)];
    }
    ...
    ) . html_tag('tr') . html_tag('th', _('What to Scan:'), 'right', '', 'nowrap') . html_tag('td') . '<select name="filters_spam_scan_set">' . '<option value=""';
    ...
    echo '>' . _('Only unread messages') . '</option>' . '</select>' . '</td>' . '</tr>' . html_tag(
        'tr',
        html_tag('td', '&nbsp;') . html_tag(
                'td',
                _(
    ...
    foreach ($spam_filters as $Key => $Value) {
        ...
        echo '</td></tr>' . html_tag(
            'tr',
            html_tag('td', '&nbsp;') . html_tag('td', $spam_filters[$Key]['comment'], 'left')
        ) . "\n";
    }
...
if (!isset($_GET['action']) || 'spam' != $_GET['action']) {
    ...
    printf(_('Spam is sent to <b>%s</b>'), ($filters_spam_folder ?: _('[<i>not set yet</i>]')));

    echo '<br>';

    printf(_('Spam scan is limited to <b>%s</b>'), (('new' == $filters_spam_scan) ? _('New Messages Only') : _('All Messages')));
