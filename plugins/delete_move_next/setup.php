 ...
 *
 * $Id: setup.php,v 1.23 2003/03/08 04:41:35 jangliss Exp $
 */
...
function delete_move_expunge_from_all($id)
{
    ...
    for ($i = 0, $iMax = count($msort); $i < $iMax; $i++) {
        if ($msgs[$i]['ID'] == $id) {
            $delAt = $i;
        } elseif ($msgs[$i]['ID'] > $id) {
            if (!$uid_support) {
...
function delete_move_next_read($currloc)
{
    ...
    if (!(($where && $what) || (-1 == $currentArrayIndex)) && !$passed_ent_id) {
        ...
        * echo "<base href=\"$location/\">" . */
        ...
        if (('on' == $delete_move_next_formATtop) && ('top' == $currloc)) {
            if ($next_if_del > 0) {
        ...
        if (('off' != $delete_move_next_formATbottom) && ('bottom' == $currloc)) {
            if ($next_if_del > 0) {
                delete_move_next_moveNextForm($next_if_del);
            } else {
                delete_move_next_moveRightMainForm();
...
{
    global $color, $where, $what, $currentArrayIndex, $passed_id, $mailbox, $sort, $startMessage, $delete_id, $move_id, $imapConnection;
    ...
    echo '</select> ' . '<input type="submit" value="' . _('Move') . '">' . '</small>' . '</form>' . '</td>' . '</tr>';
}
...
{
    global $username, $data_dir, $delete_move_next_t, $delete_move_next_formATtop, $delete_move_next_b, $delete_move_next_formATbottom;
    ...
    echo '> ' . _('Display at top') . ' <input type=checkbox name=delete_move_next_formATtopi';

    if ('on' == $delete_move_next_formATtop) {
        echo ' checked';
    }
    ...
    echo '> ' . _('Display at bottom') . '<input type=checkbox name=delete_move_next_formATbottomi';

    if ('off' != $delete_move_next_formATbottom) {
        echo ' checked';
    }

    echo '> ' . _('with move option') . '<br>' . "</td></tr>\n";
}
