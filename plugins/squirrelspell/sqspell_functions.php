 ...
 *
 * $Id: sqspell_functions.php,v 1.14 2003/03/11 15:57:22 kink Exp $
 *
 * @author  Konstantin Riabitsev <icon@duke.edu> ($Author: kink $)
 * @version $Date: 2003/03/11 15:57:22 $
 * @param mixed $title
 * @param mixed $scriptsrc
 * @param mixed $body
 */
 ...
 *
 * @param The   $title     title of the page to display
 * @param  This $scriptsrc is used to link a file.js into the
 ...
 * @param The   $body      body of the message to display.
 */
    ...
    if ('options_main' != $MOD) {
        echo html_tag(
            ...
            "\n" . html_tag('td', '<hr>', 'left')
                 ...
                 "\n" . html_tag(
                     'td',
                     '<a href="sqspell_options.php">' . _('Back to &quot;SpellChecker Options&quot; page') . '</a>',
                     'center'
                 )
             ) . "\n";
    }
 ...
 * customized.
 *
 * @param  Used  $onload    to indicate and pass the name of a js function
 *                    to call in a <body onload="function()" for automatic
 *                    onload script execution.
 * @param  Title $title     of the page.
 * @param  If    $scriptsrc defined, link this javascript source page into
 *                    the document using <script src="file.js"> format.
 * @param The    $body      content to include.
 */
 ...
 * information.
 *
 * @param  A       $mode  string with either of the two recognized values:
 *                "encrypt" or "decrypt".
 * @param The      $ckey  key to use for processing (the user's password
 *                in our case.
 * @param  Content $input to decrypt or encrypt, according to $mode.
...
function sqspell_crypto($mode, $ckey, $input)
{
    ...
    switch ($mode) {
        ...
        case 'decrypt':
             ...
             */
            if (!mb_strstr($crypto, '# SquirrelSpell')) {
                $crypto = 'PANIC';
            }
            break;
    }
 ...
 *
 * @param  Contents $words_string of the 0.2-style user dictionary.
 ...
 *
 * @param The $words contents of the user's ".words" file.
 * @return false|string[] strings array with dictionaries available
 ...
 *
 * @param The    $words contents of the user's ".words" file.
 * @param  Which $lang  language words to return, e.g. requesting
...
function sqspell_getLang($words, $lang)
{
    ...
    $end = mb_strpos($words, '#', $start + 1);

    $lang_words = mb_substr($words, $start, $end - $start);
...
function sqspell_getWords()
{
    ...
    if (file_exists($SQSPELL_WORDS_FILE)) {
        ...
        $fp = fopen($SQSPELL_WORDS_FILE, 'rb');
    ...
    if ($words && !mb_strstr($words, '# SquirrelSpell')) {
        /**
        ...
        if ('PANIC' == $words) {
            ...
            $msg = html_tag(
                'p',
                "\n" . '<strong>' . _('ATTENTION:') . '</strong><br>' . _(
                           "SquirrelSpell was unable to decrypt your personal dictionary. This is most likely due to the fact that you have changed your mailbox password. In order to proceed, you will have to supply your old password so that SquirrelSpell can decrypt your personal dictionary. It will be re-encrypted with your new password after this.<br>If you haven't encrypted your dictionary, then it got mangled and is no longer valid. You will have to delete it and start anew. This is also true if you don't remember your old password -- without it, the encrypted data is no longer accessible."
                       ),
                'left'
            ) . "\n" . '<blockquote>' . "\n" . '<form method="post" onsubmit="return AYS()">' . "\n" . '<input type="hidden" name="MOD" value="crypto_badkey">' . "\n" . html_tag(
                       'p',
                       "\n" . '<input type="checkbox" name="delete_words" value="ON">' . _('Delete my dictionary and start a new one') . '<br>' . _('Decrypt my dictionary with my old password:') . '<input name="old_key" size=\"10\">',
                       'left'
                   ) . "\n" . '</blockquote>' . "\n" . html_tag(
                       'p',
            ...
            if (mb_strstr($SCRIPT_NAME, 'sqspell_options')) {
                sqspell_makePage(
                    _('Error Decrypting Dictionary'),
                    'decrypt_error.js',
                    $msg
                );
            } else {
    ...
    if (mb_strstr($words, 'Dictionary v0.2')) {
 ...
 *
 * @param The $words contents of the ".words" file to write.
 */
function sqspell_writeWords($words)
{
    ...
    $fp = fopen($SQSPELL_WORDS_FILE, 'wb');
 ...
 * slashes.
 *
 * @param  the $rMOD name of the module requested to include.
