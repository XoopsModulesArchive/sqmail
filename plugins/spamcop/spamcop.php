 ...
 * *
 * *  $Id: spamcop.php,v 1.14 2003/03/27 04:02:12 cigamit Exp $
 **/
define('SM_PATH', '../../');
...
function getMessage_RFC822_Attachment(
    ...
    if ('OK' == $response) {
        ...
        if ('../' == mb_substr($attachment_dir, 0, 3)) {
            $attachment_dir = mb_substr($attachment_dir, 3);
...
if ('quick_email' == $spamcop_method
    || 'thorough_email' == $spamcop_method) {
    ...
    if (!isset($compose_messages[$session]) || (null === $compose_messages[$session])) {
    ...
    $composeMessage = getMessage_RFC822_Attachment(
        $message,
        $composeMessage,
        $passed_id,
        $passed_ent_id = '',
    ...
    ?>
        ...
        ?>
                <form method="post" action="../../src/right_main.php">
                    <input type="hidden" name="mailbox" value="<?php echo htmlspecialchars($mailbox, ENT_QUOTES | ENT_HTML5) ?>">
                    <input type="hidden" name="startMessage" value="<?php echo htmlspecialchars($startMessage, ENT_QUOTES | ENT_HTML5) ?>">
        ...
        <td align="right" valign="top">
            <?php if ('thorough_email' == $spamcop_method
                      || 'quick_email' == $spamcop_method) {
                        if ('thorough_email' == $spamcop_method) {
            ...
            <form method="post" action="<?php echo $form_action ?>">
                <input type="hidden" name="mailbox" value="<?php echo htmlspecialchars($mailbox, ENT_QUOTES | ENT_HTML5) ?>">
                <input type="hidden" name="spamcop_is_composing" value="<?php echo htmlspecialchars($passed_id, ENT_QUOTES | ENT_HTML5) ?>">
                ...
                <?php
                        ...
                        fwrite($imap_stream, $sid . ' FETCH ' . $passed_id . ' RFC822' . "\r\n");
                        ...
                        if (mb_strlen($Message) > 50000) {
                            ...
                            $Message = mb_substr($Message, 0, 50000 - mb_strlen($Warning)) . $Warning;
                        }
                ...
                <form method="post" action="http://spamcop.net/sc" name="submitspam"
                    ...
                    <form method="post" action="http://spamcop.net/sc" name="submitspam"
                        ...
                        echo htmlspecialchars($Message, ENT_QUOTES | ENT_HTML5); ?>">
