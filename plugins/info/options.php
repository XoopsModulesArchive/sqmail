 ...
 * jason@stdbev.com
 *
 * $Id: options.php,v 1.6 2003/03/11 15:57:16 kink Exp $
 *
 */
...
$default_charset = mb_strtoupper($default_charset);
...
if (!isset($submit) || 'default' == $submit) {
    print '<BR><SMALL><FONT COLOR='
          . $color[6]
...
foreach ($type as $index => $value) {
    ...
    if ('TEST_0' == $index && ('default' == $submit || '' == $submit)) {
        print ' CHECKED';
    }

    $check = 'CHECK_' . $index;

    if (isset($$check) && 'clear' != $submit && 'default' != $submit) {
        print ' CHECKED';
    }
...
if ('submit' == $submit) {
    ...
    for ($i = 0, $iMax = count($tests); $i < $iMax; $i++) {
        print '<CENTER><TABLE WIDTH="95%" BORDER=0 BGCOLOR=' . $color[4] . ">\n";

        print '<TR><TD><B>' . $tests[$i] . '</B></TD><TR>';

        print '<TR><TD><SMALL><B><FONT COLOR=' . $color[7] . ">Request:</FONT></SMALL></B></TD></TR>\n";

        $response = imap_test($imap_stream, $type[$tests[$i]]);

        print '<TR><TD><SMALL><B><FONT COLOR=' . $color[7] . ">Response:</FONT></SMALL></B></TD></TR>\n";
