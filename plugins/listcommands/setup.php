 ...
 *
 * $Id: setup.php,v 1.24 2003/03/10 16:18:29 kink Exp $
 */
...
{
    global $passed_id, $passed_ent_id, $color, $mailbox, $message, $compose_new_win;
    ...
    foreach ($message->rfc822_header->mlist as $cmd => $actions) {
        ...
        if ((function_exists('array_key_exists')
             && /* PHP >= 4.1 */ !array_key_exists($cmd, $fieldsdescr))
            || (function_exists('key_exists')
                && !array_key_exists($cmd, $fieldsdescr))            /* PHP == 4.0.6 */) {
            continue;
        }
