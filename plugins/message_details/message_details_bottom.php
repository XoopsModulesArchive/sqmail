 ...
 *
 * $Id: message_details_bottom.php,v 1.10 2003/03/11 15:57:19 kink Exp $
 */
define('SM_PATH', '../../');
...
function CalcEntity($entString, $direction)
{
    ...
    if (-1 == $direction) {
        $pos = mb_strrpos($entString, '.');

        $result = mb_substr($entString, 0, $pos);
    }
    ...
    switch ($direction) {
        case 0:
            ...
            if (false === $pos) {
                ...
                $level = mb_substr($entString, 0, $pos);

                $sublevel = mb_substr($entString, $pos + 1);

                $sublevel++;

                $result = (string)$level . '.' . (string)$sublevel;
            }
            break;
        case 1:
            $result = (string)$entString . '.0';
            break;
        default:
            break;
    }
...
$count = count($body);
$body[$count - 1] = mb_substr($body[$count - 1], -1);
for ($i = 1; $i < $count; $i++) {
    $line = trim($body[$i]);
    ...
    if (($header || $messageheader) && (preg_match('/^.*boundary="?(.+(?=")|.+).*/i', $line, $reg))) {
        ...
        $boundaries[] = ['bnd' => $bnd, 'bndreg' => $bndreg];
        ...
        if ('' == $entStr) {
            ...
            $entStr = CalcEntity((string)$entStr, 1);
        }
    }
    ...
    if (('' != $line && '-' == $line[0] || $header) && isset($boundaries[0])) {
        ...
        if (preg_match($regstr, $line, $reg)) {
            ...
            if ($bndend) {
                ...
                $entStr = CalcEntity((string)$entStr, 0);
                ...
                $pre .= "\n \n" . '</div>' . "\n \n" . '<div class="entheader" ID="' . $entStr . 'H"><a name="' . (string)$entStr . '"><b><font color =' . "$color[2]>";
            ...
            if ($header) {
                if (!$contentset && preg_match("/^.*(content-type:)\s*(\w+)\/(\w+).*/i", $line, $reg)) {
                    if ('message' == mb_strtolower($reg[2]) && 'rfc822' == mb_strtolower($reg[3])) {
                        $messageheader = true;
                    }
                    ...
                    if ('image' == $reg[2]) {
                        $entities[(string)$entStr] = [];

                        $entities[(string)$entStr]['entity'] = $entStr;

                        $entities[(string)$entStr]['contenttype'] = $reg[2] . '/' . $reg[3];
                    }
                } elseif (!$nameset && preg_match("/^.*(name=\s*)\"(.*)\".*/i", $line, $reg)) {
                    $name = htmlspecialchars($reg[2], ENT_QUOTES | ENT_HTML5);

                    $content[$content_indx]['name'] = decodeHeader($name);

                    $nameset = true;

                    if (isset($entities[(string)$entStr])) {
                        $entities[(string)$entStr]['name'] = urlencode($reg[2]);
                    }
                } elseif (preg_match("/^.*(content-transfer-encoding:)\s*(\w+-?(\w+)?).*/i", $line, $reg)) {
                    $encoding = $reg[2];

                    if (isset($entities[(string)$entStr])) {
                        $entities[(string)$entStr]['encoding'] = $reg[2];
                    }
    ...
    */
    ...
    $message_body .= (string)$pre . (string)$line . (string)$end . '<BR>' . "\r\n";
}
