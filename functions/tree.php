 ...
 *
 * $Id: tree.php,v 1.25 2002/12/31 12:49:32 kink Exp $
 */
...
function findParentForChild($value, $treeIndexToStart, $tree)
{
    ...
    if ((isset($tree[$treeIndexToStart])) && (mb_strstr($value, $tree[$treeIndexToStart]['value']))) {
        ...
        if ($tree[$treeIndexToStart]['doIHaveChildren']) {
            ...
            for ($i = 0, $iMax = count($tree[$treeIndexToStart]['subNodes']); $i < $iMax; $i++) {
...
function addChildNodeToTree($comparisonValue, $value, &$tree)
{
    ...
    if (false === $tree[$parentNode]['doIHaveChildren']) {
...
function walkTreeInPreOrderEmptyTrash($index, $imap_stream, $tree)
{
    ...
    if ($tree[$index]['doIHaveChildren']) {
        for ($j = 0, $jMax = count($tree[$index]['subNodes']); $j < $jMax; $j++) {
...
function walkTreeInPostOrderCreatingFoldersUnderTrash($index, $imap_stream, $tree, $topFolderName)
{
    ...
    $position = mb_strrpos($topFolderName, $delimiter) + 1;

    $subFolderName = mb_substr($tree[$index]['value'], $position);
