 ...
 *
 * $Id: mailbox_display.php,v 1.321 2003/03/27 16:47:38 philippe_mingo Exp $
 */
...
function elapsed($start)
{
    ...
    [$start2, $start1] = explode(' ', $start);

    [$end2, $end1] = explode(' ', $end);
...
function printMessageInfo(
    ...
    global $checkall, $color, $msgs, $msort, $td_str, $msg, $default_use_priority, $message_highlight_list, $index_order, $indent_array,   /* indent subject by */ $pos,            /* Search postion (if any)  */ $thread_sort_messages, /* thread sorting on/off */ $server_sort_order, /* sort value when using server-sorting */ $row_count, $allow_server_sort; /* enable/disable server-side sorting */
    ...
    if (count($senderNames)) {
        foreach ($senderNames as $senderNames_part) {
            ...
            if ($senderNames_part[1]) {
                ...
                $senderName .= htmlspecialchars($senderNames_part[0], ENT_QUOTES | ENT_HTML5);
            }
        }
    }
    ...
    if (isset($msg['FLAG_FLAGGED']) && (true === $msg['FLAG_FLAGGED'])) {
    ...
    if (!isset($msg['FLAG_SEEN']) || (false === $msg['FLAG_SEEN'])) {
        $bold = '<b>';
    ...
    if (is_array($message_highlight_list) && count($message_highlight_list)) {
        foreach ($message_highlight_list as $message_highlight_list_part) {
            if ('' != trim($message_highlight_list_part['value'])) {
                $high_val = mb_strtolower($message_highlight_list_part['value']);

                $match_type = mb_strtoupper($message_highlight_list_part['match_type']);

                if ('TO_CC' == $match_type) {
                    foreach ($msg['TO'] as $address) {
                        if (mb_strstr('^^' . mb_strtolower($address[0]), $high_val)
                            || mb_strstr('^^' . mb_strtolower($address[1]), $high_val)) {
                    ...
                    if (mb_strstr('^^' . mb_strtolower($msg[$match_type]), $high_val)) {
                        $hlt_color = $message_highlight_list_part['color'];
        ...
        foreach ($index_order as $index_order_part) {
            switch ($index_order_part) {
                ...
                case 2: /* from */ echo html_tag(
                    ...
                    break;
                case 3: /* date */ $date_string = $msg['DATE_STRING'] . '';
                    if ('' == $date_string) {
                        $date_string = _('Unknown date');
                    }
                    echo html_tag(
                        'td',
                        $bold . $flag . $fontstr . $date_string . $fontstr_end . $flag_end . $bold_end,
                        'center',
                        $hlt_color,
                        'nowrap'
                    );
                    break;
                case 4: /* subject */ $td_str = $bold;
                    if (1 == $thread_sort_messages) {
                    ...
                    break;
                case 5: /* flags */ $stuff = false;
                    $td_str = '<b><small>';

                    if (isset($msg['FLAG_ANSWERED']) && true === $msg['FLAG_ANSWERED']) {
                        $td_str .= _('A');
                    ...
                    if ($default_use_priority) {
                        if ((1 == $msg['PRIORITY']) || (2 == $msg['PRIORITY'])) {
                    ...
                    if (isset($msg['FLAG_DELETED']) && true === $msg['FLAG_DELETED']) {
                    ...
                    break;
                case 6: /* size */ echo html_tag(
                    'td',
                    $bold . $fontstr . show_readable_size($msg['SIZE']) . $fontstr_end . $bold_end,
                    'right',
                    $hlt_color
                );
                    break;
...
function calc_msort($msgs, $sort)
{
    ...
    } elseif ((2 == $sort) || (3 == $sort)) {
        foreach ($msgs as $item) {
            $msort[] = $item['FROM-SORT'];
        }
    } elseif ((4 == $sort) || (5 == $sort)) {
        foreach ($msgs as $item) {
...
function displayMessageArray(
    ...
    if (1 == $thread_sort_messages && $num_msgs) {
...
function mail_message_listing_beginning(
    ...
    if (mb_strpos($php_self, 'move_messages.php')) {
        $php_self = str_replace('move_messages.php', 'webmail.php', $php_self);
    }
    ...
    $moveFields = '<input type="hidden" name="msg" value="' . htmlspecialchars($msg, ENT_QUOTES | ENT_HTML5) . '">' . '<input type="hidden" name="mailbox" value="' . htmlspecialchars($mailbox, ENT_QUOTES | ENT_HTML5) . '">' . '<input type="hidden" name="startMessage" value="' . htmlspecialchars(
            $start_msg,
    ...
    echo html_tag('tr') . "\n" . html_tag('td', '', 'left', '', '') . html_tag('table', '', '', $color[9], 'border="0" width="100%" cellpadding="1"  cellspacing="0"') . '<tr><td>' . html_tag('table', '', '', $color[0], 'border="0" width="100%" cellpadding="1"  cellspacing="0"') . html_tag(
        'tr',
        getSmallStringCell(_('Move Selected To'), 'left', 'nowrap') . getSmallStringCell(_('Transform Selected Messages'), 'right')
    ...
    if (true === $allow_thread_sort) {
        ...
        echo html_tag(
            ...
            html_tag(
                'td',
                '&nbsp;<a href=' . $source_url . '?sort=' . (string)$sort . '&start_messages=1&set_thread=' . (string)$set_thread . '&mailbox=' . urlencode($mailbox) . '><small>' . $thread_name . '</a></small>&nbsp;',
                '',
                '',
                ''
            ),
            ...
            html_tag(
                ...
                html_tag(
                    ...
                    html_tag(
                        ...
                        html_tag(
                            'tr',
                            html_tag('td', $paginator_str) . html_tag('td', $msg_cnt_str, 'right')
                        ),
                        '',
                        $color[4],
...
function printHeader($mailbox, $sort, $color, $showsort = true)
{
    ...
    $widths = [1 => 1, 2 => 25, 3 => 5, 4 => 0, 5 => 1, 6 => 5];
    ...
    foreach ($index_order as $item) {
        switch ($item) {
            case 1: /* checkbox */
            case 5: /* flags */ echo html_tag('td', '&nbsp;', '', '', 'width="1%"');
                break;
            case 2: /* from */ if (handleAsSent($mailbox)) {
                ...
                echo "</td>\n";
                break;
            case 3: /* date */ echo html_tag('td', '', 'left', '', 'width="5%" nowrap') . '<b>' . _('Date') . '</b>';
                if ($showsort) {
                    ShowSortButton($sort, $mailbox, 0, 1);
                }
                echo "</td>\n";
                break;
            case 4: /* subject */ echo html_tag('td', '', 'left', '', 'width="' . $subjectwidth . '%"') . '<b>' . _('Subject') . '</b>';
                if ($showsort) {
                    ShowSortButton($sort, $mailbox, 4, 5);
                }
                echo "</td>\n";
                break;
            case 6: /* size */ echo html_tag('td', '<b>' . _('Size') . '</b>', 'center', '', 'width="5%" nowrap');
                break;
        }
    }
...
function ShowSortButton($sort, $mailbox, $Up, $Down)
{
    ...
    echo ' <a href="' . $source_url . '?newsort=' . $which . '&amp;startMessage=1&amp;mailbox=' . urlencode($mailbox) . '"><IMG SRC="../images/' . $img . '" BORDER=0 WIDTH=12 HEIGHT=10 ALT="sort"></a>';
}
...
function get_selectall_link($start_msg, $sort)
{
    ...
    if ($javascript_on) {
        $result = '<script language="JavaScript" type="text/javascript">'
                  ...
                  . "}\n"
                  . "//-->\n"
                  . '</script><a href="#" onClick="CheckAll();">'
                  . _('Toggle All')
                  . "</a>\n";
    } else {
        if (mb_strpos($PHP_SELF, '?')) {
            $result .= "<a href=\"$PHP_SELF&amp;mailbox=" . urlencode($mailbox) . "&amp;startMessage=$start_msg&amp;sort=$sort&amp;checkall=";
        } else {
            $result .= "<a href=\"$PHP_SELF?mailbox=" . urlencode($mailbox) . "&amp;startMessage=$start_msg&amp;sort=$sort&amp;checkall=";
        }

        if (isset($checkall) && '1' == $checkall) {
            $result .= '0';
        } else {
            $result .= '1';
        }
...
function get_msgcnt_str($start_msg, $end_msg, $num_msgs)
{
    ...
    if ($start_msg < $end_msg) {
        $result = sprintf(
            _('Viewing Messages: <B>%s</B> to <B>%s</B> (%s total)'),
            $start_msg,
            $end_msg,
            $num_msgs
        );
...
function get_paginator_link($box, $start_msg, $use, $text)
{
    ...
    $result = "<A HREF=\"webmail.php?use_mailbox_cache=$use" . "&amp;startMessage=$start_msg&amp;mailbox=$box\" " . "TARGET=\"right\">$text</A>";
...
function get_paginator_str(
    ...
    } elseif (($next_grp > $num_msgs) && ($prev_grp >= 0)) {
        $prv_str = get_paginator_link($box, $prev_grp, $use, _('Previous'));
    ...
    if ($pg_sel && ($num_msgs > $show_num)) {
        ...
        $cur_pg = (int)($start_msg / $show_num) + 1;
        ...
        /* Otherwise, compute some magic to choose the four quarters. */
            ...
            } elseif (($cur_pg - $q2_pgs - ceil($q2_pgs / 3)) <= $q1_pgs) {
                $extra_pgs = $q2_pgs;
            ...
            } elseif (($cur_pg + $q3_pgs + 1) >= ($tot_pgs - $q4_pgs + 1)) {
                $extra_pgs = $q3_pgs;
    ...
    } elseif (999999 == $PG_SHOWNUM) {
        $pg_str = '<A HREF="webmail.php?PG_SHOWALL=0' . "&amp;use_mailbox_cache=$use&amp;startMessage=1&amp;mailbox=$box\" " . 'TARGET="right">' . _('Paginate') . '</A>' . $spc;
    }

    /* If necessary, compute the 'show all' string. */

    if (('' != $prv_str) || ('' != $nxt_str)) {
        $all_str = '<A HREF="webmail.php?PG_SHOWALL=1' . "&amp;use_mailbox_cache=$use&amp;startMessage=1&amp;mailbox=$box\" " . 'TARGET="right">' . _('Show All') . '</A>';
    }
...
function processSubject($subject, $threadlevel = 0)
{
    ...
    if (mb_strlen($subject) <= $trim_at) {
        return $subject;
    }

    $ent_strlen = $orig_len = mb_strlen($subject);
    ...
    while ($ent_loc < $trim_val && (false !== ($ent_loc = mb_strpos($subject, '&', $ent_offset)))
           && (false !== ($ent_loc_end = mb_strpos($subject, ';', $ent_loc + 3)))) {
    ...
    if (($trim_val > 50) && (mb_strlen($subject) > ($trim_val)) && (mb_strpos($subject, ';', $trim_val) < ($trim_val + 6))) {
        $i = mb_strpos($subject, ';', $trim_val);

        if ($i) {
            $trim_val = mb_strpos($subject, ';', $trim_val);
        }
    }
    ...
    return mb_substr($subject, 0, $trim_val) . '...';
}
...
function getMbxList($imapConnection)
{
    ...
    echo sqimap_mailbox_option_list($imapConnection, [mb_strtolower($lastTargetMailbox)]);
