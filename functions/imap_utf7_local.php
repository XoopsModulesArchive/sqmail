...
function imap_utf7_decode_local($s)
{
    ...
    for ($i = 0, $len = mb_strlen($s); $i < $len; $i++) {
        ...
        if (mb_strlen($b64_s) > 0) {
            if ('-' == $c) {
                if ('&' == $b64_s) {
                    $iso_8859_1_s .= '&';
                } else {
                    $iso_8859_1_s .= decodeBASE64(mb_substr($b64_s, 1));
                }
...
function encodeBASE64($s)
{
    ...
    for ($i = 0, $iMax = mb_strlen($s); $i < $iMax; $i++) {
        ...
        if (0 == $p) {
            $e .= mb_substr($B64Chars, ((ord($c) & 252) >> 2), 1);

            $t = (ord($c) & 3);

            $p = 1;
        } elseif (1 == $p) {
            $e .= $B64Chars[($t << 4) + ((ord($c) & 240) >> 4)];

            $t = (ord($c) & 15);

            $p = 2;
        } elseif (2 == $p) {
            $e .= $B64Chars[($t << 2) + ((ord($c) & 192) >> 6)];
