 ...
 *
 * $Id: attachment_common.php,v 1.25 2003/03/09 18:45:06 ebullient Exp $
 */
...
function register_attachment_common($type, $func)
{
    ...
    $squirrelmail_plugin_hooks['attachment ' . $type]['attachment_common'] = 'attachment_common_' . $func;
}
...
function attachment_common_link_text(&$Args)
{
    ...
    $Args[1]['attachment_common']['href'] = set_url_var(
...
{
    $Args[1]['attachment_common']['href'] = SM_PATH . 'src/read_body.php?startMessage=' . $Args[2] . '&amp;passed_id=' . $Args[3] . '&amp;mailbox=' . $Args[4] . '&amp;passed_ent_id=' . $Args[5] . '&amp;override_type0=message&amp;override_type1=rfc822';
...
function attachment_common_link_html(&$Args)
{
    ...
    $Args[1]['attachment_common']['href'] = SM_PATH . 'src/view_text.php?' . $QUERY_STRING . /* why use the overridetype? can this be removed */
...
function attachment_common_octet_stream(&$Args)
{
    ...
    $Ret = do_hook(
        'attachment ' . $FileExtensionToMimeType[$Ext],
        $Args[1],
        $Args[2],
        $Args[3],
        $Args[4],
