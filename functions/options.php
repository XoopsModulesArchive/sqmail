 ...
 *
 * $Id: options.php,v 1.34 2003/03/25 16:07:22 jangliss Exp $
 */
...
class SquirrelOption
{
    ...
    public function __construct($name, $caption, $type, $refresh_level, $possible_values = '')
    {
        /* Set the basic stuff. */
        ...
        if ((SMOPT_TYPE_HIDDEN != $type) && (SMOPT_TYPE_COMMENT != $type)) {
    ...
    public function setSaveFunction($save_function)
    {
        $this->save_function = $save_function;
    }

    public function createHTMLWidget()
    {
        ...
        switch ($this->type) {
            ...
            case SMOPT_TYPE_FLDRLIST:
                ...
                break;
            default:
                $result = '<font color="' . $color[2] . '">' . sprintf(_("Option Type '%s' Not Found"), $this->type) . '</font>';
        }
    ...
    public function createWidget_String()
    {
        switch ($this->size) {
    ...
    public function createWidget_StrList()
    {
    ...
    public function createWidget_FolderList()
    {
        $selected = [mb_strtolower($this->value)];
    ...
    public function createWidget_TextArea()
    {
        switch ($this->size) {
            case SMOPT_SIZE_TINY:
                $rows = 3;
        ...
        $result = "<textarea name=\"new_$this->name\" rows=\"$rows\" " . "cols=\"$cols\">$this->value</textarea>";

        return ($result);
    }

    public function createWidget_Integer()
    {
        return $this->createWidget_String();
    }

    public function createWidget_Float()
    {
        return $this->createWidget_String();
    }

    public function createWidget_Boolean()
    {
        /* Do the whole current value thing. */

        if (SMPREF_NO != $this->value) {
            $yes_chk = ' checked';
        ...
        $yes_option = '<input type="radio" name="new_' . $this->name . '" value="' . SMPREF_YES . "\"$yes_chk>&nbsp;" . _('Yes');

        /* Build the no choice. */

        $no_option = '<input type="radio" name="new_' . $this->name . '" value="' . SMPREF_NO . "\"$no_chk>&nbsp;" . _('No');
    ...
    public function createWidget_Hidden()
    {
        $result = '<input type="hidden" name="new_' . $this->name . '" value="' . $this->value . '">';

        return ($result);
    }

    public function createWidget_Comment()
    {
        $result = $this->comment;
...
function print_option_groups($option_groups)
{
    ...
    foreach ($option_groups as $next_optgrp) {
        ...
        if ('' != $next_optgrp['name']) {
            echo html_tag(
                ...
                "\n" . html_tag(
                         ...
                         'center',
                         '',
                         'valign="middle" colspan="2" nowrap'
                     )
            ) . "\n";
        }
        ...
        foreach ($next_optgrp['options'] as $option) {
            if (SMOPT_TYPE_HIDDEN != $option->type) {
                echo html_tag(
                    'tr',
...
function OptionSubmit($name)
{
    echo html_tag(
        'tr',
        html_tag('td', '&nbsp;', 'left', '', 'colspan="2"') . html_tag('td', '<input type="submit" value="' . _('Submit') . '" name="' . $name . '">', 'left', '', 'colspan="2"')
    ) . "\n";
}
