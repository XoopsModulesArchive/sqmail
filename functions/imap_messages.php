 ...
 *
 * $Id: imap_messages.php,v 1.134 2003/03/01 01:35:10 stekkel Exp $
 * @param $imap_stream
 * @param $start
 * @param $end
 * @param $mailbox
...
{
    global $default_charset, $thread_sort_messages, $internal_date_sort, $server_sort_array, $sent_folder, $mailbox, $uid_support;
...
function sqimap_get_php_sort_order($imap_stream, $mbxresponse)
{
    ...
    if ($uid_support) {
        ...
        if (isset($uids[0])) {
            if (preg_match("/^\* SEARCH (.+)$/", $uids[0], $regs)) {
                $php_sort_array = preg_preg_split('/ /', trim($regs[1]));
            }
        }
        if (false === strpos($response, "OK")) {
            $php_sort_array = 'no';
...
function get_parent_level($imap_stream)
{
     ...
     */
    for ($i = 0, $iMax = count($thread_new); $i < $iMax; $i++) {
        $thread_new[$i] = preg_replace("/\s\(/", '(', $thread_new[$i]);
        $thread_new[$i] = preg_replace("/(\d+)/", '$1|', $thread_new[$i]);
        $thread_new[$i] = preg_preg_split("/\|/", $thread_new[$i], -1, PREG_SPLIT_NO_EMPTY);
    }
    $indent_array = [];
    if (!$thread_new) {
        $thread_new = [];
...
function get_thread_sort($imap_stream)
{
    ...
    fwrite($imap_stream, $thread_query);
    ...
    if (isset($thread_list)) {
        $thread_temp = preg_preg_split('//', $thread_list, -1, PREG_SPLIT_NO_EMPTY);
    }
    ...
    $thread_list       = str_replace(')', ' ', $thread_list);
    $thread_list       = preg_preg_split("/\s/", $thread_list, -1, PREG_SPLIT_NO_EMPTY);
...
function sqimap_get_small_header_list($imap_stream, $msg_list)
{
     ...
     */
    for ($i = 0, $iMax = count($msg_list); $i < $iMax; $i++) {
    ...
    foreach ($readin_list as $r) {
        if (!$uid_support) {
            if (!preg_match("/^\\*\s+([0-9]+)\s+FETCH/iAU", $r[0], $regs)) {
                ...
                echo '<br><b><font color=$color[2]>' . _('ERROR : Could not complete request.') . '</b><br>' . _('Unknown response from IMAP server: ') . ' 1.' . htmlspecialchars($r[0], ENT_QUOTES | ENT_HTML5) . "</font><br>\n";
            } elseif (!isset($id2index[$regs[1]]) || !count($id2index[$regs[1]])) {
                set_up_language($squirrelmail_language);
                echo '<br><b><font color=$color[2]>' . _('ERROR : Could not complete request.') . '</b><br>' . _('Unknown message number in reply from server: ') . htmlspecialchars($regs[1], ENT_QUOTES | ENT_HTML5) . "</font><br>\n";
            } else {
            ...
            if (!preg_match("/^\\*\s+([0-9]+)\s+FETCH.*UID\s+([0-9]+)\s+/iAU", $r[0], $regs)) {
                set_up_language($squirrelmail_language);
                echo '<br><b><font color=$color[2]>' . _('ERROR : Could not complete request.') . '</b><br>' . _('Unknown response from IMAP server: ') . ' 1.' . htmlspecialchars($r[0], ENT_QUOTES | ENT_HTML5) . "</font><br>\n";
            } elseif (!isset($id2index[$regs[2]]) || !count($id2index[$regs[2]])) {
                set_up_language($squirrelmail_language);
                echo '<br><b><font color=$color[2]>' . _('ERROR : Could not complete request.') . '</b><br>' . _('Unknown message number in reply from server: ') . htmlspecialchars($regs[2], ENT_QUOTES | ENT_HTML5) . "</font><br>\n";
            } else {
            ...
            if ($read_part{0} == '*') {
                ...
                if ($firstchar == 'T') {
                    ...
                    if (strtolower($read_part{1}) == 'c') {
                        ...
                        $id         = 4;
                    } elseif (strtolower($read_part{1}) == 'o') {
                        $regpattern = $patterns[6];
                        $id         = 7;
                    }
                } elseif ($firstchar == 'D' && !$internaldate) {
...
function sqimap_get_message($imap_stream, $id, $mailbox)
{
    ...
    if ($read) {
        if (preg_match('/.+FLAGS\s\((.*)\)\s/AUi', $read[0], $regs)) {
            if (trim($regs[1])) {
                $flags = preg_preg_split('/ /', $regs[1], -1, 'PREG_SPLIT_NI_EMPTY');
            }
        }
    } else {
