 ...
 *
 * $Id: i18n.php,v 1.129 2003/03/27 16:47:37 philippe_mingo Exp $
 */
...
function charset_decode($charset, $string)
{
    ...
    if (preg_match('iso-8859-([[:digit:]]+)', $charset, $res)) {
        if ('1' == $res[1]) {
...
function charset_decode_iso_8859_2($string)
{
    ...
    if ('iso-8859-2' == mb_strtolower($default_charset)) {
        return $string;
    }
...
function charset_decode_iso_8859_4($string)
{
    ...
    if ('iso-8859-4' == mb_strtolower($default_charset)) {
        return $string;
    }
...
function charset_decode_iso_8859_7($string)
{
    ...
    if ('iso-8859-7' == mb_strtolower($default_charset)) {
        return $string;
    }
...
function charset_decode_iso_8859_9($string)
{
    ...
    if ('iso-8859-9' == mb_strtolower($default_charset)) {
        return $string;
    }
...
function charset_decode_iso_8859_13($string)
{
    ...
    if ('iso-8859-13' == mb_strtolower($default_charset)) {
        return $string;
    }
...
function charset_decode_windows_1254($string)
{
    ...
    if ('windows-1254' == mb_strtolower($default_charset)) {
        return $string;
    }
...
function charset_decode_windows_1253($string)
{
    ...
    if ('windows-1253' == mb_strtolower($default_charset)) {
        return $string;
    }
...
function charset_decode_windows_1257($string)
{
    ...
    if ('windows-1257' == mb_strtolower($default_charset)) {
        return $string;
    }
...
function set_up_language($sm_language, $do_search = false)
{
    ...
    global $use_gettext, $languages, $squirrelmail_language, $squirrelmail_default_language, $sm_notAlias;
    ...
    if ($do_search && !$sm_language && isset($accept_lang)) {
        $sm_language = mb_substr($accept_lang, 0, 2);
    }
    ...
    if (isset($sm_language)
        ...
        if ('ja_JP' == $squirrelmail_language && function_exists('mb_detect_encoding')) {
...
if (7 == $gettext_flags) {
    ...
    require_once SM_PATH . 'functions/gettext.php';
} else {
...
function charset_decode_utf8($string)
{
    ...
    */ global $default_charset, $languages, $sm_notAlias;
    ...
    if ('utf-8' == mb_strtolower($default_charset)) {
        return $string;
    }

    if ('utf-8' == mb_strtolower($languages[$sm_notAlias]['CHARSET'])) {
        return $string;
    }
...
function japanese_charset_xtra()
{
    ...
    if (function_exists('mb_detect_encoding')) {
        switch (func_get_arg(0)) { /* action */ case 'decode':
            $detect_encoding = @mb_detect_encoding($ret, mb_detect_order(), true);
            if ('JIS' == $detect_encoding
                || 'EUC-JP' == $detect_encoding
                || 'SJIS' == $detect_encoding
                || 'UTF-8' == $detect_encoding) {
            ...
            case 'encodeheader':
                ...
                if (mb_strlen($ret) > 0) {
                    ...
                    $prevcsize = mb_strlen($tmpstr);

                    for ($i = 1, $iMax = mb_strlen($ret); $i < $iMax; $i++) {
                        $tmp = mb_substr($ret, $i, 1);

                        if (mb_strlen($tmp) == $prevcsize) {
                            $tmpstr .= $tmp;
                        } else {
                            if (1 == $prevcsize) {
                                $result .= $tmpstr;
                            ...
                            $prevcsize = mb_strlen($tmp);
                        }
                    }

                    if (mb_strlen($tmpstr)) {
                        if (1 == mb_strlen(mb_substr($tmpstr, 0, 1))) {
                            $result .= $tmpstr;
                        } else {
                            $result .= str_replace(
                                ' ',
            ...
            case 'downloadfilename':
                ...
                if (false !== mb_strstr($useragent, 'Windows')
                    || false !== mb_strstr($useragent, 'Mac_')) {
            ...
            case 'wordwrap':
                ...
                $no_end = "\x5c\x24\x28\x5b\x7b\xa1\xf2\x5c\xa1\xc6\xa1\xc8\xa1\xd2\xa1" . "\xd4\xa1\xd6\xa1\xd8\xa1\xda\xa1\xcc\xa1\xf0\xa1\xca\xa1\xce\xa1\xd0\xa1\xef";
                $wrap = func_get_arg(2);

                if (mb_strlen($ret) >= $wrap
                    && '>' != mb_substr($ret, 0, 1)
                    && false === mb_strpos($ret, 'http://')
                    && false === mb_strpos($ret, 'https://')
                    && false === mb_strpos($ret, 'ftp://')) {
                    ...
                    while ($ptr < mb_strlen($ret) - 1) {
                        ...
                        while (0 != mb_strlen($l) && false !== mb_strpos($no_begin, $l)) {
                            $tmp .= $l;
...
function korean_charset_xtra()
{
    ...
    if ('downloadfilename' == func_get_arg(0)) { /* action */
        $ret = str_replace("\x0D\x0A", '', $ret);  /* Hanmail's CR/LF Clear */

        for ($i = 0, $iMax = mb_strlen($ret); $i < $iMax; $i++) {
            if ($ret[$i] >= "\xA1" && $ret[$i] <= "\xFE") {   /* 0xA1 - 0XFE are Valid */
                $i++;

                continue;
            } elseif (($ret[$i] >= 'a' && $ret[$i] <= 'z')
                      || /* From Original ereg_replace in download.php */ ($ret[$i] >= 'A' && $ret[$i] <= 'Z')
                      || ('.' == $ret[$i])
                      || ('-' == $ret[$i])) {
                continue;
            }  
