 ...
 *
 * $Id: date.php,v 1.46 2003/03/01 22:28:20 stekkel Exp $
 */
...
function getGMTSeconds($stamp, $tzc)
{
    ...
    if ('-' == mb_substr($tzc, 0, 1)) {
        $neg = true;
    } elseif ('+' != mb_substr($tzc, 0, 1)) {
        $tzc = '+' . $tzc;
    }

    $hh = mb_substr($tzc, 1, 2);

    $mm = mb_substr($tzc, 3, 2);
...
function date_intl($date_format, $stamp)
{
    ...
    $ret = str_replace('$1', mb_substr(getDayName(date('w', $stamp)), 0, 3), $ret);

    $ret = str_replace('$5', mb_substr(getMonthName(date('m', $stamp)), 0, 3), $ret);
...
function getLongDateString($stamp)
{
    ...
    if (SMPREF_TIME_12HR == $hour_format) {
