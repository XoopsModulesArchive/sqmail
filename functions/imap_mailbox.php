 ...
 *
 * $Id: imap_mailbox.php,v 1.172 2003/03/10 16:31:57 kink Exp $
 */
...
class mailboxes
{
    public $mailboxname_full = '';

    public $mailboxname_sub = '';

    public $is_noselect = false;

    public $is_special = false;

    public $is_root = false;

    public $is_inbox = false;
    ...
    public function addMbx($mbx, $delimiter, $start, $specialfirst)
    {
...
function isBoxBelow($subbox, $parentbox)
{
    ...
    if (mb_strlen($subbox) < mb_strlen($parentbox)) {
        return false;
    }

    /* check for delimiter */

    if (!mb_substr($parentbox, -1) == $delimiter) {
        $parentbox .= $delimiter;
    }

    if (mb_substr($subbox, 0, mb_strlen($parentbox)) == $parentbox) {
        return true;
    }
...
function sqimap_mailbox_select($imap_stream, $mailbox)
{
    ...
    for ($i = 0, $cnt = count($read); $i < $cnt; $i++) {
        if (preg_match('/^\*\s+OK\s\[(\w+)\s(\w+)\]/', $read[$i], $regs)) {
            $result[mb_strtoupper($regs[1])] = $regs[2];
        } elseif (preg_match('/^\*\s([0-9]+)\s(\w+)/', $read[$i], $regs)) {
            $result[mb_strtoupper($regs[2])] = $regs[1];
        } else {
            if (preg_match('/PERMANENTFLAGS(.*)/i', $read[$i], $regs)) {
                $regs[1] = trim(preg_replace(["/\(/", "/\)/", "/\]/"], '', $regs[1]));
...
function sqimap_mailbox_create($imap_stream, $mailbox, $type)
{
    ...
    if ('noselect' == mb_strtolower($type)) {
        $mailbox .= $delimiter;
    }

    $read_ary = sqimap_run_command(
        $imap_stream,
        "CREATE \"$mailbox\"",
        true,
        $response,
...
function sqimap_mailbox_delete($imap_stream, $mailbox)
{
    ...
    do_hook_function('rename_or_delete_folder', $args = [$mailbox, 'delete', '']);
...
function sqimap_mailbox_rename($imap_stream, $old_name, $new_name)
{
    if ($old_name != $new_name) {
        ...
        if (mb_substr($old_name, -1) == $delimiter) {
            $old_name = mb_substr($old_name, 0, -1);

            $new_name = mb_substr($new_name, 0, -1);
        ...
        do_hook_function('rename_or_delete_folder', $args = [$old_name, 'rename', $new_name]);

        $l = mb_strlen($old_name) + 1;

        $p = 'unformatted';

        foreach ($boxesall as $box) {
            if (mb_substr($box[$p], 0, $l) == $old_name . $delimiter) {
                $new_sub = $new_name . $delimiter . mb_substr($box[$p], $l);

                if ('cyrus' == $imap_server_type) {
                    $cmd = 'RENAME "' . $box[$p] . '" "' . $new_sub . '"';

                    $data = sqimap_run_command(
                        $imap_stream,
                        $cmd,
                        true,
                        $response,
...
function sqimap_mailbox_parse($line, $line_lsub)
{
    ...
    for ($g = 0, $cnt = count($line); $g < $cnt; ++$g) {
        ...
        $dm_count = mb_substr_count($mailbox, $delimiter);

        if (mb_substr($mailbox, -1) == $delimiter) {
        ...
        if (('inbox' == mb_strtolower(mb_substr($mailbox, 0, 5)))
            || (mb_substr($mailbox, 0, mb_strlen($folder_prefix)) == $folder_prefix)
            || (isset($boxesallbyname[$parentfolder])
                && (mb_strlen($parentfolder) > 0))) {
            $indent = $dm_count - (mb_substr_count($folder_prefix, $delimiter));
        ...
        if (mb_substr($mailbox, -1) == $delimiter) {
            $mailbox = mb_substr($mailbox, 0, -1);
        }

        $boxesall[$g]['unformatted'] = $mailbox;

        if (mb_substr($mailbox, 0, mb_strlen($folder_prefix)) == $folder_prefix) {
            $mailbox = mb_substr($mailbox, mb_strlen($folder_prefix));
        }
        ...
        if (isset($line[$g])) {
            ...
            $flags = trim(mb_strtolower(str_replace('\\', '', $regs[1])));
...
function sqimap_mailbox_option_list($imap_stream, $show_selected = 0, $folder_skip = 0, $boxes = 0)
{
    ...
    foreach ($boxes as $boxes_part) {
        if (!in_array('noselect', $boxes_part['flags'], true)) {
            $box = $boxes_part['unformatted'];

            $lowerbox = mb_strtolower($box);

            if (0 != $folder_skip && in_array($lowerbox, $folder_skip, true)) {
                continue;
            }

            if ('inbox' == $lowerbox) {
                $box2 = _('INBOX');
            } elseif (2 == $shorten_box_names) {  /* delimited, style = 2 */
                $box2 = str_replace('&nbsp;&nbsp;', '.&nbsp;', $boxes_part['formatted']);
            } elseif (1 == $shorten_box_names) {     /* indent, style = 1 */
            ...
            if (0 != $show_selected && in_array($lowerbox, $show_selected, true)) {
...
function sqimap_mailbox_list($imap_stream)
{
    ...
    if (!isset($boxesnew)) {
         ...
         * (larger then fgets buffer)
            ...
            if (!$inbox_subscribed && 'INBOX' == mb_strtoupper($temp_mailbox_name)) {
        ...
        for ($i = 0, $iMax = count($sorted_lsub_ary); $i < $iMax; $i++) {
            if (mb_substr($sorted_lsub_ary[$i], -1) == $delimiter) {
                $mbx = mb_substr($sorted_lsub_ary[$i], 0, -1);
            } else {
        ...
        for ($k = 0; $k < $cnt; ++$k) {
            if ('inbox' == mb_strtolower($boxesall[$k]['unformatted'])) {
                $boxesnew[] = $boxesall[$k];
...
function sqimap_mailbox_list_all($imap_stream)
{
    ...
    fwrite($imap_stream, $ssid . " LIST \"$folder_prefix\" *\r\n");
    ...
    $fld_pre_length = mb_strlen($folder_prefix);
    ...
    for ($i = 0, $cnt = count($read_ary); $i < $cnt; $i++) {
        ...
        if (mb_substr($read_ary[$i], 0, $lsid) != $ssid) {
            /* Store the raw IMAP reply */
            ...
            if ((eregi('^inbox' . quotemeta($delimiter), $mailbox))
                ...
                || (isset($boxesallbyname[$parentfolder]) && (mb_strlen($parentfolder) > 0))) {
                if ($dm_count) {
            ...
            $boxes[$g]['unformatted-disp'] = mb_substr($mailbox, $fld_pre_length);
            ...
            /* Another workaround for EIMS */ //            if (isset($read_mlbx[1]) &&
            ...
            $flags = mb_substr($read_mlbx, mb_strpos($read_mlbx, '(') + 1);

            $flags = mb_substr($flags, 0, mb_strpos($flags, ')'));

            $flags = str_replace('\\', '', $flags);

            $flags = trim(mb_strtolower($flags));
    ...
    if (!isset($boxesnew)) {
        ...
        for ($i = 0; $i < $cnt; $i++) {
            ...
            */
            ...
            if (mb_substr($mbx, -1) == $delimiter) {
                $mbx = mb_substr($mbx, 0, -1);
            }

            $sorted_lsub_ary[] = ['mbx' => $mbx, 'noselect' => $noselect];
        }

        array_multisort($sorted_lsub_ary, SORT_ASC, SORT_REGULAR);

        for ($i = 0; $i < $cnt; $i++) {
            if ('INBOX' == $sorted_lsub_ary[$i]['mbx']) {
                $inbox_in_list = true;
...
function sqimap_fill_mailbox_tree($mbx_ary, $mbxs = false)
{
    ...
    $special_folders = ['INBOX', $sent_folder, $draft_folder, $trash_folder];
    ...
    if (isset($folder_prefix) && '' != $folder_prefix) {
        $start = mb_substr_count($folder_prefix, $delimiter);

        if (mb_strrpos($folder_prefix, $delimiter) == (mb_strlen($folder_prefix) - 1)) {
            $trail_del = true;

            $mailboxes->mailboxname_full = mb_substr($folder_prefix, 0, (mb_strlen($folder_prefix) - 1));
        } else {
    ...
    for ($i = 0; $i < $cnt; $i++) {
        if ('' != $mbx_ary[$i]['mbx']) {
            ...
            $r_del_pos = mb_strrpos($mbx_ary[$i]['mbx'], $delimiter);

            if ($r_del_pos) {
                $mbx->mailboxname_sub = mb_substr($mbx_ary[$i]['mbx'], $r_del_pos + 1);
