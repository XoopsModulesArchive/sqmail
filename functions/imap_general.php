 ...
 *
 * $Id: imap_general.php,v 1.140 2003/04/01 20:46:59 kink Exp $
 */
...
function sqimap_run_command_list($imap_stream, $query, $handle_errors, &$response, &$message, $unique_id = false)
{
    ...
    $string = "<b><font color=$color[2]>\n" . _('ERROR : No available imapstream.') . "</b></font>\n";
...
function sqimap_fgets($imap_stream)
{
    ...
    while (false === mb_strpos($results, "\r\n", $offset)) {
        if (!($read = fgets($imap_stream, $buffer))) {
            break;
        }

        if ('' != $results) {
            $offset = mb_strlen($results) - 1;
        }
...
function sqimap_read_data_list($imap_stream, $pre, $handle_errors, &$response, &$message, $query = '')
{
    ...
    if (false === $handle_errors) {
        ...
        if (false === mb_strstr($message, 'command resulted in')) {
            set_up_language($squirrelmail_language);

            require_once SM_PATH . 'functions/display_messages.php';

            $string = "<b><font color=$color[2]>\n" . _('ERROR : Could not complete request.') . "</b><br>\n" . _('Query:') . ' ' . htmlspecialchars($query, ENT_QUOTES | ENT_HTML5) . '<br>' . _('Reason Given: ') . htmlspecialchars($message, ENT_QUOTES | ENT_HTML5) . "</font><br>\n";
        ...
        $string = "<b><font color=$color[2]>\n" . _('ERROR : Bad or malformed request.') . "</b><br>\n" . _('Query:') . ' ' . htmlspecialchars($query, ENT_QUOTES | ENT_HTML5) . '<br>' . _('Server responded: ') . htmlspecialchars($message, ENT_QUOTES | ENT_HTML5) . "</font><br>\n";
...
function sqimap_read_data($imap_stream, $pre, $handle_errors, &$response, &$message, $query = '')
{
    ...
    if (isset($res[1]) && is_array($res[1]) && isset($res[1][0])
        ...
        foreach ($res as $index => $value) {
            $result = array_merge($result, $res[(string)$index]);
        }
    }
...
function sqimap_login($username, $password, $imap_server_address, $imap_port, $hide)
{
    ...
    if ((true === $use_imap_tls) and (check_php_version(4, 3)) and (extension_loaded('openssl'))) {
    ...
    if (!$imap_stream) {
        if (!$hide) {
            ...
            $string = sprintf(
                _('Error connecting to IMAP server: %s.') . "<br>\r\n",
                $imap_server_address
    ...
    if (('cram-md5' == $imap_auth_mech) or ('digest-md5' == $imap_auth_mech)) {
        // We're using some sort of authentication OTHER than plain or login

        $tag = sqimap_session_id(false);

        if ('digest-md5' == $imap_auth_mech) {
            $query = $tag . " AUTHENTICATE DIGEST-MD5\r\n";
        } elseif ('cram-md5' == $imap_auth_mech) {
            $query = $tag . " AUTHENTICATE CRAM-MD5\r\n";
        }

        fwrite($imap_stream, $query);
        ...
        if ('+' == $response[0]) {
            ...
            fwrite($imap_stream, $reply);

            $read = sqimap_fgets($imap_stream);

            if ('digest-md5' == $imap_auth_mech) {
                // DIGEST-MD5 has an extra step..
                if ('+' == mb_substr($read, 0, 1)) { // OK so far..
                    fwrite($imap_stream, "\r\n");
    ...
    } elseif ('login' == $imap_auth_mech) {
        // Original IMAP login code

        $query = 'LOGIN "' . quoteimap($username) . '" "' . quoteimap($password) . '"';

        $read = sqimap_run_command($imap_stream, $query, false, $response, $message);
    } elseif ('plain' == $imap_auth_mech) {
            ...
            if ('NO' != $response) {
                ...
                if (isset($read) && is_array($read)) {
                    ...
                    foreach ($read as $line) {
                        $string .= htmlspecialchars($line, ENT_QUOTES | ENT_HTML5) . "<br>\n";
                    }
                }
...
function sqimap_capability($imap_stream, $capability = '')
{
    ...
    if ($capability) {
        return $sqimap_capabilities[$capability] ?? false;
    }
...
function sqimap_get_delimiter($imap_stream = false)
{
    ...
    if ((!empty($optional_delimiter)) && 'detect' != $optional_delimiter) {
    ...
    if (!$sqimap_delimiter) {
        if (sqimap_capability($imap_stream, 'NAMESPACE')) {
            ...
            fwrite($imap_stream, ". LIST \"INBOX\" \"\"\r\n");

            $read = sqimap_read_data($imap_stream, '.', true, $a, $b);

            $quote_position = mb_strpos($read[0], '"');

            $sqimap_delimiter = mb_substr($read[0], $quote_position + 1, 1);
        }
    }
...
function sqimap_get_num_messages($imap_stream, $mailbox)
{
    ...
    for ($i = 0, $iMax = count($read_ary); $i < $iMax; $i++) {
...
function parseAddress($address, $max = 0, $addr_ar = [], $group = '', $host = '')
{
    $pos = 0;
    ...
    while ($pos < $j) {
        ...
        switch ($char) {
            case '=':
                if (preg_match('/^(=\?([^?]*)\?(Q|B)\?([^?]*)\?=)(.*)/Ui', mb_substr($address, $pos), $reg)) {
                    if (!$personal) {
                        $personal = mb_substr($address, 0, $pos);
                    }

                    $personal .= $reg[1];

                    $pos += mb_strlen($personal);
                }
                ++$pos;
                break;
            case '"': /* get the personal name */ ++$pos;
                if ('"' == $address[$pos]) {
                    ++$pos;
                } else {
                    ...
                    while ($pos < $j) {
                        ...
                        if (($personal_end - 2) > 0
                            ...
                            $personal = mb_substr($address, $personal_start, $personal_end - $personal_start);
            ...
            case '(':  /* rip off comments */ $addr_start = $pos;
                ...
                $pos = $addr_start + 1;
                break;
            case ',':  /* we reached a delimiter */ if ('' == $addr) {
                $addr = mb_substr($address, 0, $pos);
            } elseif ('' == $personal) {
                $personal = trim(mb_substr($address, 0, $addr_start));
            }
                if (!$personal && $comment) {
                    $personal = $comment;
                }
...
function sqimap_unseen_messages($imap_stream, $mailbox)
{
    ...
    while (isset($read_ary[$i])) {
        if (preg_match('UNSEEN ([0-9]+)', $read_ary[$i], $regs)) {
            break;
        }
...
function sqimap_status_messages($imap_stream, $mailbox)
{
    ...
    return ['MESSAGES' => $messages, 'UNSEEN' => $unseen, 'RECENT' => $recent];
}
...
{
    fwrite($imap_stream, sqimap_session_id() . " APPEND \"$sent_folder\" (\\Seen) \{$length}\r\n");
...
{
    if ('map:' != mb_substr($imap_server, 0, 4)) {
        return $imap_server;
    }

    $function = mb_substr($imap_server, 4);
...
function map_yp_alias($username)
{
    ...
    return rtrim(mb_substr($yp, mb_strlen($username) + 1));
}
