 ...
 *
 * $Id: search.php,v 1.92 2003/03/27 16:47:47 philippe_mingo Exp $
 */
if (!isset($xoopsIntro)) {
...
function get_recent($username, $data_dir)
{
    ...
    $types = ['search_what', 'search_where', 'search_folder'];
...
function get_saved($username, $data_dir)
{
    ...
    $types = ['saved_what', 'saved_where', 'saved_folder'];

    foreach ($types as $key) {
        for ($x = 1; ; $x++) {
            $saved_attributes[$key][$x] = getPref($data_dir, $username, $key . (string)$x, '');
...
function update_recent($what, $where, $mailbox, $username, $data_dir)
{
    ...
    for ($i = 1, $iMax = count($attributes['search_what']); $i <= $iMax; $i++) {
...
function save_recent($save_index, $username, $data_dir)
{
    ...
    $saved_types = [0 => 'saved_what', 1 => 'saved_where', 2 => 'saved_folder'];
...
if (!isset($mailbox) || 'None' == $mailbox || '' == $mailbox) {
    $mailbox = $boxes[0]['unformatted'];
}
if ('All Folders' == $mailbox) {
    $search_all = 'all';
}

if (isset($composenew) && $composenew) {
    $comp_uri = '../src/compose.php?mailbox=' . urlencode($mailbox) . "&amp;session=$composesession&amp;attachedmessages=true&amp";
...
if (!isset($submit)) {
    $submit = '';
} elseif ($submit == _('Search') && !empty($what)) {
    if ($recent_count > 0) {
        update_recent($what, $where, $mailbox, $username, $data_dir);
    }
} elseif ('forget' == $submit) {
...
/* Saved Search Table */
if ($saved_count > 0) {
    echo "<br>\n" . html_tag('table', '', 'center', $color[9], 'width="95%" cellpadding="1" cellspacing="1" border="0"') . html_tag(
        'tr',
        html_tag('td', '<b>Saved Searches</b>', 'center')
    ) . html_tag('tr') . html_tag('td') . html_tag('table', '', 'center', '', 'width="100%" cellpadding="2" cellspacing="2" border="0"');
    ...
    for ($i = 0; $i < $saved_count; ++$i) {
        ...
        echo html_tag('td', imap_utf7_decode_local($saved_attributes['saved_folder'][$i]), 'left', '', 'width="35%"')
             ...
             . '<a href=search.php'
             . '?mailbox='
             . htmlspecialchars($saved_attributes['saved_folder'][$i], ENT_QUOTES | ENT_HTML5)
             . '&amp;what='
             . htmlspecialchars($saved_attributes['saved_what'][$i], ENT_QUOTES | ENT_HTML5)
             . '&amp;where='
             . htmlspecialchars($saved_attributes['saved_where'][$i], ENT_QUOTES | ENT_HTML5)
             . '>'
             . _('edit')
             . '</a>'
             . '&nbsp;|&nbsp;'
...
if ($recent_count > 0) {
    ...
    ) . html_tag('tr') . html_tag('td') . html_tag('table', '', 'center', '', 'width="100%" cellpadding="0" cellspacing="0" border="0"');
    ...
    for ($i = 1; $i <= $recent_count; ++$i) {
        ...
        if (isset($attributes['search_what'][$i])
            ...
            echo html_tag('td', imap_utf7_decode_local($attributes['search_folder'][$i]), 'left', '', 'width="35%"')
                 . html_tag('td', htmlspecialchars($attributes['search_what'][$i], ENT_QUOTES | ENT_HTML5), 'left')
...
$show_selected = [mb_strtolower($mailbox)];
...
if ('all' == $search_all) {
    ...
    echo '<BR><CENTER><B>' . _('Search Results') . "</B></CENTER><BR>\n";

    for ($x = 0; $x < $boxcount; $x++) {
        if (!in_array('noselect', $boxes[$x]['flags'], true)) {
            $mailbox = $boxes[$x]['unformatted'];
        }

        if (($submit == _('Search') || 'Search_no_update' == $submit) && !empty($what)) {
            ...
            $perbox_count[] = $count_all;
        }
    }

    for ($i = 0, $iMax = count($perbox_count); $i < $iMax; $i++) {
        if ($perbox_count[$i]) {
            $count_all = true;
...
} /*  search one folder option  */ else {
    if (($submit == _('Search') || 'Search_no_update' == $submit) && !empty($what)) {
        echo '<br>' . html_tag('div', '<b>' . _('Search Results') . '</b>', 'center') . "\n";
...
if ($submit == _('Search') && empty($what)) {
    echo '<br>' . html_tag('div', '<b>Please enter something to search for</b>', 'center') . "\n";
}
