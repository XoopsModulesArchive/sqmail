 ...
 *
 * $Id: options_highlight.php,v 1.47 2003/01/19 11:40:23 kink Exp $
 */
if (!isset($xoopsIntro)) {
...
echo html_tag(
    ...
    'center',
    $color[9],
    'width="95%" border="0" cellpadding="1" cellspacing="0"'
) . "<br>\n" . html_tag('table', '', '', '', 'width="100%" border="0" cellpadding="1" cellspacing="0"') . html_tag('tr') . "\n" . html_tag('td', '', 'left');

echo '<center>[<a href="options_highlight.php?action=add">' . _('New') . '</a>]' . ' - [<a href="options.php">' . _('Done') . '</a>]</center><br>' . "\n";
if (count($message_highlight_list) >= 1) {
    echo html_tag('table', '', 'center', '', 'width="80%" border="0" cellpadding="3" cellspacing="0"') . "\n";

    for ($i = 0, $iMax = count($message_highlight_list); $i < $iMax; $i++) {
        $match_type = '';
        ...
        echo html_tag(
            ...
            html_tag(
                     'td',
                     '<small>[<a href="options_highlight.php?action=edit&amp;theid=' . $i . '">' . _('Edit') . '</a>]&nbsp;[<a href="options_highlight.php?action=delete&amp;theid=' . $i . '">' . _('Delete') . '</a>]</small>',
                     'left',
                     $color[4],
                     'width="20%" nowrap'
                 ) . html_tag(
                     'td',
                     htmlspecialchars($message_highlight_list[$i]['name'], ENT_QUOTES | ENT_HTML5),
                     'left'
                 ) . html_tag(
                     'td',
                     $match_type . ' = ' . htmlspecialchars($message_highlight_list[$i]['value'], ENT_QUOTES | ENT_HTML5),
                     'left'
                 ),
            '',
...
if ('edit' == $action || 'add' == $action) {
    ...
    if ('edit' == $action && isset($theid) && isset($message_highlight_list[$theid]['color'])) {
        for ($i = 0; $i < 14; $i++) {
    ...
    if (!isset($message_highlight_list[$theid]['color'])) {
        $selected_choose = ' checked';
    } elseif ($pre_defined_color) {
        $selected_predefined = ' checked';
    } elseif ('' == $selected_choose) {
    ...
    if ('edit' == $action && isset($theid) && isset($message_highlight_list[$theid]['name'])) {
        $disp = $message_highlight_list[$theid]['name'];
    } else {
        $disp = '';
    }

    $disp = htmlspecialchars($disp, ENT_QUOTES | ENT_HTML5);
    ...
    oh_opt(
        'from',
        (isset($theid) ? 'from' == $message_highlight_list[$theid]['match_type'] : 1),
        _('From')
    );

    oh_opt(
        'to',
        (isset($theid) ? 'to' == $message_highlight_list[$theid]['match_type'] : 0),
        _('To')
    );

    oh_opt(
        'cc',
        (isset($theid) ? 'cc' == $message_highlight_list[$theid]['match_type'] : 0),
        _('Cc')
    );

    oh_opt(
        'to_cc',
        (isset($theid) ? 'to_cc' == $message_highlight_list[$theid]['match_type'] : 0),
        _('To or Cc')
    );

    oh_opt(
        'subject',
        (isset($theid) ? 'subject' == $message_highlight_list[$theid]['match_type'] : 0),
        _('Subject')
    );
    ...
    if ('edit' == $action && isset($theid) && isset($message_highlight_list[$theid]['value'])) {
    ...
    echo '         <input type="text" value="' . $disp . '" name="value" size=40>';
