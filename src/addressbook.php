 ...
 *
 * $Id: addressbook.php,v 1.58 2003/03/27 16:47:46 philippe_mingo Exp $
 */
if (!isset($xoopsIntro)) {
...
function adressbook_inp_field($label, $field, $name, $size, $values, $add)
{
    ...
    if (isset($values[$field])) {
        $td_str .= htmlspecialchars(strip_tags($values[$field]), ENT_QUOTES | ENT_HTML5);
    }
    ...
    if ('ja_JP' == $squirrelmail_language) {
        echo html_tag(
            ...
            adressbook_inp_field(
                 ...
                 ) . adressbook_inp_field(_('E-mail address'), 'email', $name, 45, $values, '') . adressbook_inp_field(_('Last name'), 'lastname', $name, 45, $values, '') . adressbook_inp_field(_('First name'), 'firstname', $name, 45, $values, '') . adressbook_inp_field(
                     _('Additional info'),
                     'label',
                     $name,
                     45,
                     ...
                     html_tag(
                         'td',
                         '<INPUT TYPE=submit NAME="' . $name . '[SUBMIT]" VALUE="' . $submittext . '">',
                         'center',
                         $color[4],
                         'colspan="2"'
                     )
            ...
            'center',
            '',
            'border="0" cellpadding="1" width="90%"'
        ) . "\n";
    } else {
        echo html_tag(
            'table',
            adressbook_inp_field(
                     ...
                     $values,
                     ' <SMALL>' . _('Must be unique') . '</SMALL>'
                 ) . adressbook_inp_field(_('E-mail address'), 'email', $name, 45, $values, '') . adressbook_inp_field(_('First name'), 'firstname', $name, 45, $values, '') . adressbook_inp_field(_('Last name'), 'lastname', $name, 45, $values, '') . adressbook_inp_field(
                     _('Additional info'),
                     'label',
                     $name,
                     45,
...
if (sqgetGlobalVar('REQUEST_METHOD', $req_method, SQ_SERVER) && 'POST' == $req_method) {
    /**************************************************
     * Add new address                                *
     **************************************************/
    ...
    if (!empty($addaddr['nickname'])) {
        ...
        if ((!empty($deladdr)) && count($sel) > 0) {
            ...
            for ($i = 0; (($i < count($sel)) && !$delfailed); $i++) {
                [$sbackend, $snick] = explode(':', $sel[$i]);

                /* When we get to a new backend, process addresses in *
                 * previous one.                                      */

                if ($prevback != $sbackend && -1 != $prevback) {
            ...
            if (!empty($editaddr)) {
                ...
                if (isset($sel) && count($sel) > 0) {
                    if (count($sel) > 1) {
                        ...
                        echo '<INPUT TYPE=hidden NAME=oldnick VALUE="' . htmlspecialchars($olddata['nickname'], ENT_QUOTES | ENT_HTML5)
                             . "\">\n" . '<INPUT TYPE=hidden NAME=backend VALUE="' . htmlspecialchars($olddata['backend'], ENT_QUOTES | ENT_HTML5)
                        ...
                        if (!$r) {
                            ...
                            echo html_tag(
                                ...
                                html_tag(
                                    ...
                                    html_tag(
                                        'td',
                                        "\n" . '<br><strong><font color="' . $color[2] . '">' . _('ERROR') . ': ' . $abook->error . '</font></strong>' . "\n",
                                        'center'
                                    )
                                ),
                                'center',
                            ...
                            echo '<INPUT TYPE=hidden NAME=oldnick VALUE="' . htmlspecialchars($oldnick, ENT_QUOTES | ENT_HTML5)
                                 . "\">\n" . '<INPUT TYPE=hidden NAME=backend VALUE="' . htmlspecialchars($backend, ENT_QUOTES | ENT_HTML5)
                                 . "\">\n" . '<INPUT TYPE=hidden NAME=doedit VALUE=1>' . "\n" . '</FORM>';
        ...
        while (list($undef, $row) = each($alist)) {
            ...
            if ($prevbackend != $row['backend']) {
                ...
                echo html_tag(
                    ...
                    '',
                    'width="95%"'
                ) . "\n" . html_tag('table', '', 'center', '', 'border="0" cellpadding="1" cellspacing="0" width="90%"') . html_tag(
                         'tr',
                         "\n" . html_tag('th', '&nbsp;', 'left', '', 'width="1%"') . html_tag('th', _('Nickname'), 'left', '', 'width="1%"') . html_tag('th', _('Name'), 'left', '', 'width="1%"') . html_tag('th', _('E-mail'), 'left', '', 'width="1%"') . html_tag(
                             'th',
                             _('Info'),
                             'left',
                             '',
            ...
            if (in_array($row['backend'] . ':' . $row['nickname'], $defselected, true)) {
                $selected = 'CHECKED';
            } else {
                $selected = '';
            }
            ...
            if ('ja_JP' == $squirrelmail_language) {
                echo html_tag('tr', '') . html_tag(
                    'td',
                    '<SMALL>' . '<INPUT TYPE=checkbox ' . $selected . ' NAME="sel[]" VALUE="' . $row['backend'] . ':' . $row['nickname'] . '"></SMALL>',
                    'center',
                    '',
                    'valign="top" width="1%"'
                ) . html_tag('td', '&nbsp;' . $row['nickname'] . '&nbsp;', 'left', '', 'valign="top" width="1%" nowrap') . html_tag('td', '&nbsp;' . $row['lastname'] . ' ' . $row['firstname'] . '&nbsp;', 'left', '', 'valign="top" width="1%" nowrap') . html_tag(
                        'td',
                        '',
                        'left',
                        '',
            ...
            echo htmlspecialchars($row['email'], ENT_QUOTES | ENT_HTML5) . '</A>&nbsp;</td>' . "\n" . html_tag('td', '&nbsp;' . htmlspecialchars($row['label'], ENT_QUOTES | ENT_HTML5) . '&nbsp;', 'left', '', 'valign="top" width="1%"') . "</tr>\n";
