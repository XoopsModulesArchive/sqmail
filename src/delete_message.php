 ...
 *
 * $Id: delete_message.php,v 1.46 2003/03/11 17:39:13 kink Exp $
 */
if (!isset($xoopsIntro)) {
...
if (isset($where) && isset($what)) {
    header(
        "Location: $location/search.php?where=" . $where . '&what=' . $what . '&mailbox=' . urlencode($mailbox)
    );
} else {
    if (!empty($saved_draft) || !empty($mail_sent)) {
        ...
        header(
            "Location: $location/webmail.php?sort=$sort&startMessage=$startMessage&mailbox=" . urlencode($mailbox)
        );
    }
}
