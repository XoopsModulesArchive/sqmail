 ...
 *
 * $Id: download.php,v 1.107 2003/04/01 16:49:45 stekkel Exp $
 */
if (!isset($xoopsIntro)) {
...
if (mb_strlen($filename) < 1) {
    if ('plain' == $type1 && 'text' == $type0) {
        $suffix = 'txt';

        $filename = $subject . '.txt';
    } elseif ('richtext' == $type1 && 'text' == $type0) {
        $suffix = 'rtf';

        $filename = $subject . '.rtf';
    } elseif ('postscript' == $type1 && 'application' == $type0) {
        $suffix = 'ps';

        $filename = $subject . '.ps';
    } elseif ('rfc822' == $type1 && 'message' == $type0) {
        $suffix = 'eml';
...
function DumpHeaders($type0, $type1, $filename, $force)
{
    ...
    if (false !== mb_strstr($HTTP_USER_AGENT, 'compatible; MSIE ')
        && false === mb_strstr($HTTP_USER_AGENT, 'Opera')) {
        $isIE = 1;
    }

    if (false !== mb_strstr($HTTP_USER_AGENT, 'compatible; MSIE 6')
        && false === mb_strstr($HTTP_USER_AGENT, 'Opera')) {
        $isIE6 = 1;
    }
    ...
    if (isset($languages[$squirrelmail_language]['XTRA_CODE'])
        ...
        $filename = $languages[$squirrelmail_language]['XTRA_CODE']('downloadfilename', $filename, $HTTP_USER_AGENT);
    } else {
