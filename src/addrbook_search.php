 ...
 *
 * $Id: addrbook_search.php,v 1.49 2003/03/03 05:44:34 ebullient Exp $
 */
if (!isset($xoopsIntro)) {
...
function display_result($res, $includesource = true)
{
    ...
    while (list($undef, $row) = each($res)) {
        ...
        echo html_tag('tr', '', '', $tr_bgcolor, 'nowrap') . html_tag(
            'td',
            '<small><a href="javascript:to_address(' . "'" . $email . "');\">To</A> | " . '<a href="javascript:cc_address(' . "'" . $email . "');\">Cc</A> | " . '<a href="javascript:bcc_address(' . "'" . $email . "');\">Bcc</A></small>",
            'center',
            '',
            'valign="top" width="5%" nowrap'
        ) . html_tag('td', '&nbsp;' . htmlspecialchars($row['name'], ENT_QUOTES | ENT_HTML5), 'left', '', 'valign="top" nowrap') . html_tag(
                'td',
                '&nbsp;' . '<a href="javascript:to_and_close(' . "'" . $email . "');\">" . htmlspecialchars($row['email'], ENT_QUOTES | ENT_HTML5) . '</A>',
                'left',
                '',
                'valign="top"'
            ) . html_tag('td', htmlspecialchars($row['label'], ENT_QUOTES | ENT_HTML5), 'left', '', 'valign="top" nowrap');
...
require_once dirname(__DIR__) . '/functions/strings.php';
require_once dirname(__DIR__) . '/functions/addressbook.php';
...
if (empty($query) && empty($show) && empty($listall)) {
    echo html_tag(
        'p',
        '<br>' . _('No persons matching your search was found'),
        'center'
    ) . "\n", //for xoops
...
/* Create search form */
if ('form' == $show && empty($listall)) {
    echo '<FORM NAME=sform TARGET=abookres ACTION="addrbook_search.php' . '" METHOD="POST">' . "\n" . html_tag('table', '', '', '', 'border="0" width="100%" height="100%"') . html_tag('tr') . html_tag(
        'td',
        '  <strong>' . _('Search for') . "</strong>\n",
        'left',
        '',
        'nowrap valign="middle" width="10%"'
    ) . html_tag('td', '', 'left', '', '') . '<INPUT TYPE=text NAME=query VALUE="' . htmlspecialchars($query, ENT_QUOTES | ENT_HTML5) . "\" SIZE=28>\n";
    ...
    echo '</td></tr>' . html_tag(
        'tr',
        html_tag('td', '', 'left') . html_tag(
                'td',
                '<INPUT TYPE=submit VALUE="' . _('Search') . '" NAME=show>' . '&nbsp;|&nbsp;<INPUT TYPE=submit VALUE="' . _('List all') . '" NAME=listall>' . "\n" . '&nbsp;|&nbsp;<INPUT TYPE=button VALUE="' . _('Close') . '" onclick="parent.close();">' . "\n",
                'left'
            )
    ) . '</table></form>' . "\n";
} else {
    ...
    if ('blank' == $show && empty($listall)) {
        if (-1 != $backend || 'blank' == $show) {
            if ('blank' == $show) {
