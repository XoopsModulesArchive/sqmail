 ...
 *
 * $Id: options.php,v 1.121 2003/03/03 05:44:34 ebullient Exp $
 */
if (!isset($xoopsIntro)) {
...
function print_optionpages_row($leftopt, $rightopt = false)
{
    ...
    echo html_tag(
        ...
        '',
        $color[4],
        'width="100%" cellpadding="0" cellspacing="5" border="0"'
    );
}
...
if (!@is_file($optpage_file)) {
    $optpage = SMOPT_PAGE_MAIN;
} elseif (SMOPT_PAGE_MAIN != $optpage) {
...
if (SMOPT_PAGE_MAIN == $optpage) {
    ...
    if (SMOPT_MODE_SUBMIT == $optmode) {
        ...
        } elseif (SMOPT_REFRESH_FOLDERLIST == $max_refresh) {
    ...
    foreach ($optpage_blocks as $cur_optpage) {
        ...
        } elseif (SMPREF_JS_ON == $javascript_on) {
