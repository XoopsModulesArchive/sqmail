 ...
 *    - Save As Draft
 *
 * $Id: compose.php,v 1.319 2003/03/27 16:47:46 philippe_mingo Exp $
 */
...
function replyAllString($header)
{
     ...
     * 3) get the addresses.
     */
    $url_replytoall_ar = $header->getAddr_a(['to', 'cc'], $excl_ar);
...
function getReplyCitation($orig_from)
{
    ...
    switch ($reply_citation_style) {
        ...
        case 'quote_who':
            ...
            break;
        case 'user-defined':
            $start = $reply_citation_start . ($reply_citation_start == '' ? '' : ' ');
...
function getforwardHeader($orig_header)
{
    ...
    if ($orig_header->cc != [] && $orig_header->cc != '') {
...
if (!isset($session) || (isset($newmessage) && $newmessage)) {
    ...
    $session        = (string)$composesession + 1;
...
if ($draft) {
    ...
    if (!deliverMessage($composeMessage, true)) {
        ...
        if (isset($delete_draft)) {
            header(
                'Location: delete_message.php?mailbox=' . urlencode($draft_folder) . "&message=$delete_draft&sort=$sort&startMessage=1&saved_draft=yes"
            );
            exit();
        } else {
            if ($compose_new_win == '1') {
                ...
                header(
                    "Location: webmail.php?mailbox=$draft_folder&sort=$sort" . '&startMessage=1&note=' . urlencode($draft_message)
                );
                exit();
            }
        }
...
if ($send) {
    ...
    if (checkInput(false) && !isset($AttachFailure)) {
        ...
        $body = $newBody . "\n\n--------------------------------------\nsent through sqMail for Xoops (http://www.guanxiCRM.com)";
        ...
        if (isset($delete_draft)) {
            header(
                'Location: delete_message.php?mailbox=' . urlencode($draft_folder) . "&message=$delete_draft&sort=$sort&startMessage=1&mail_sent=yes"
            );
            exit();
        }
        if ($compose_new_win == '1') {
            ...
            header(
                "Location: webmail.php?mailbox=$urlMailbox&sort=$sort" . '&startMessage=1'
            );
        }
    } else {
        if ($compose_new_win == '1') {
        ...
        if (isset($AttachFailure)) {
            plain_error_message(
                _('Could not move/copy file. File not attached'),
                $color
            );
        }
        checkInput(true);
     ...
     */
    require_once __DIR__ . '/addrbook_search_html.php';
    //    header('Location: ./addrbook_search_html.php');
} elseif (isset($attach)) {
    if (saveAttachedFiles($session)) {
        plain_error_message(_('Could not move/copy file. File not attached'), $color);
    }
...
{
    global $editor_size, $default_use_priority, $body, $use_signature, $composesession, $data_dir, $username, $username, $key, $imapServerAddress, $imapPort, $compose_messages, $composeMessage;
    ...
    if ($passed_id) {
        ...
        if ($passed_ent_id) {
            ...
            */
            ...
            $entities = $message->entities[0]->findDisplayEntity([], $alt_order = ['text/plain']);
            if (!count($entities)) {
                $entities = $message->entities[0]->findDisplayEntity([], $alt_order = ['text/plain', 'html/plain']);
            }
            ...
            $entities = $message->findDisplayEntity([], $alt_order = ['text/plain']);
            if (!count($entities)) {
                $entities = $message->findDisplayEntity([], $alt_order = ['text/plain', 'html/plain']);
            }
        ...
        foreach ($entities as $ent) {
            ...
            if ($type1 == 'html') {
                $bodypart = str_replace(['&nbsp;', '&gt', '&lt'], [' ', '<', '>'], $bodypart);
        ...
        if (!empty($idents) && $idents > 1) {
            $identities[] = '"' . getPref($data_dir, $username, 'full_name') . '" <' . getPref($data_dir, $username, 'email_address') . '>';
            for ($i = 1; $i < $idents; $i++) {
                $enc_from_name = '"' . getPref($data_dir, $username, 'full_name' . $i) . '" <' . getPref($data_dir, $username, 'email_address' . $i) . '>';
        ...
        switch ($action) {
            ...
            case ('reply'):
                ...
                } elseif (is_object($send_to)) { /* unnessecarry, just for falesafe purpose */
                    $send_to = $orig_header->getAddr_s('reply_to');
                } else {
...
function showInputForm($session, $values = false)
{
    ...
    if ($idents > 1) {
        ...
        echo '   <tr>' . "\n" . html_tag('td', '', 'right', $color[4], 'width="10%"') . _('From:') . '</td>' . "\n" . html_tag('td', '', 'left', $color[4], 'width="90%"') . '         <select name="identity">' . "\n" . '         <option value="default">' . htmlspecialchars(
                $fn,
        ...
        for ($i = 1; $i < $idents; $i++) {
            ...
            echo '>' . htmlspecialchars($fn, ENT_QUOTES | ENT_HTML5);
            if ($em != '') {
                if ($fn != '') {
                    echo htmlspecialchars(' <' . $em . '>', ENT_QUOTES | ENT_HTML5) . "\n";
                } else {
    ...
    echo '   <tr>'
         ...
         . html_tag('td', '', 'left', $color[4])
         . "\n";
    echo '         <input type="text" name="subject" size="60" value="' . $subject . '">' . "\n" . '      </td>' . "\n" . '   </tr>' . "\n\n";
    ...
    if ($compose_new_win == '1') {
        echo '   <TR>' . "\n" . '      <TD BGCOLOR="' . $color[0] . '" COLSPAN=2 ALIGN=CENTER>' . "\n" . '         <TEXTAREA NAME=body ROWS=20 COLS="' . $editor_size . '" WRAP="VIRTUAL">';
    } else {
    ...
    if ((bool)ini_get('file_uploads')) {
        ...
        $configvars = ['post_max_size', 'memory_limit', 'upload_max_filesize'];
    ...
    echo '</TABLE>' . "\n" . '<input type="hidden" name="username" value="' . $username . "\">\n" . '<input type=hidden name=action value="' . $action . "\">\n" . '<INPUT TYPE=hidden NAME=mailbox VALUE="' . htmlspecialchars($mailbox, ENT_QUOTES | ENT_HTML5) . "\">\n";
    /* 
...
function showComposeButtonRow()
{
    ...
    if ($default_use_mdn) {
        if ($mdn_user_support) {
            echo '          ' . _('Receipt') . ': ' . '<input type="checkbox" name="request_mdn" value=1' . ($request_mdn == '1' ? ' checked' : '') . '>' . _('On Read') . ' <input type="checkbox" name="request_dr" value=1' . ($request_dr == '1' ? ' checked' : '') . '>' . _('On Delivery');
        }
    }
...
{
    global $send_to, $send_to_cc, $send_to_bcc, $mailprio, $subject, $body, $username, $popuser, $usernamedata, $identity, $data_dir, $request_mdn, $request_dr, $default_charset, $color, $useSendmail, $domain, $action;
    global $imapServerAddress, $imapPort, $sent_folder, $key;

    $rfc822_header = $composeMessage->rfc822_header;

    $abook                   = addressbook_init(false, true);
    $rfc822_header->to       = $rfc822_header->parseAddress($send_to, true, [], '', $domain, [&$abook, 'lookup']);
    $rfc822_header->cc       = $rfc822_header->parseAddress($send_to_cc, true, [], '', $domain, [&$abook, 'lookup']);
    $rfc822_header->bcc      = $rfc822_header->parseAddress($send_to_bcc, true, [], '', $domain, [&$abook, 'lookup']);
...
function showInputForm($session, $values = false)
{
    ...
    if ($idents > 1) {
        $fn = getPref($data_dir, $username, 'full_name');
        $em = getPref($data_dir, $username, 'email_address');
        echo '   <tr>' . "\n" . html_tag('td', '', 'right', $color[4], 'width="10%"') . _("From:") . '</td>' . "\n" . html_tag('td', '', 'left', $color[4], 'width="90%"') . '         <select name="identity">' . "\n" . '         <option value="default">' . htmlspecialchars($fn);
        if ($em != '') {
            if ($fn != '') {
