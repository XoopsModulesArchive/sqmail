 ...
 * Folders.
 *
 * $Id: folders.php,v 1.112 2003/03/11 17:39:16 kink Exp $
 */
if (!isset($xoopsIntro)) {
...
if (isset($success) && $success) {
    ...
    echo html_tag(
        ...
        html_tag(
            ...
            html_tag(
                'td',
                $td_str . '<a href="../src/left_main.php" target=left>' . _('refresh folder list') . '</a>',
                'center'
            )
        ),
        'center',
...
/** CREATING FOLDERS **/
echo html_tag('table', '', 'center', '', 'width="70%" cellpadding="4" cellspacing="0" border="0"') . html_tag(
    'tr',
...
$show_selected = 0;
$skip_folders = 0;
$server_type = mb_strtolower($imap_server_type);
if ('courier' == $server_type) {
    $show_selected = ['inbox'];

    $skip_folders = ['inbox.trash'];

    if ('INBOX.' == $default_folder_prefix) {
        $skip_folders[] = 'inbox';
    }
}

if (false === $default_sub_of_inbox) {
...
if ($show_contain_subfolders_option) {
    echo '<br><input type=CHECKBOX NAME="contain_subs"> &nbsp;' . _('Let this folder contain subfolders') . '<BR>';
}
...
$num_max = 1;
if ('courier' == mb_strtolower($imap_server_type) || $move_to_trash) {
    $num_max++;
}
if ($move_to_sent) {
    $num_max++;
...
for ($p = 0, $cnt = count($boxes); $p < $cnt && $count_special_folders < $num_max; $p++) {
    if ('inbox' == mb_strtolower($boxes[$p]['unformatted'])) {
        $count_special_folders++;
    } elseif ('courier' == mb_strtolower($imap_server_type)
              && 'inbox.trash' == mb_strtolower($boxes[$p]['unformatted'])) {
        $count_special_folders++;
    } elseif ($boxes[$p]['unformatted'] == $trash_folder && $trash_folder) {
        $count_special_folders++;
    } elseif ($boxes[$p]['unformatted'] == $sent_folder && $sent_folder) {
        $count_special_folders++;
    } elseif ($boxes[$p]['unformatted'] == $draft_folder && $draft_folder) {
        $count_special_folders++;
    }
}
...
) . html_tag('tr') . html_tag('td', '', 'center', $color[0], 'width="50%"');
...
if ($count_special_folders < count($boxes)) {
    ...
    for ($i = 0, $iMax = count($boxes); $i < $iMax; $i++) {
        ...
        if (('inbox' != mb_strtolower($boxes[$i]['unformatted']))
...
if (!$no_list_for_subscribe) {
    ...
    for ($i = 0, $q = 0, $iMax = count($boxes_all); $i < $iMax; $i++) {
        $use_folder = true;

        for ($p = 0, $pMax = count($boxes); $p < $pMax; $p++) {
            if ($boxes_all[$i]['unformatted'] == $boxes[$p]['unformatted']) {
                $use_folder = false;

                continue;
            } elseif ($boxes_all[$i]['unformatted-dm'] == $folder_prefix) {
                $use_folder = false;
            }
        }
    ...
    if ($box && $box2) {
        ...
        for ($q = 0, $qMax = count($box); $q < $qMax; $q++) {
    ...
    echo "<FORM ACTION=\"folders_subscribe.php?method=sub\" METHOD=\"POST\">\n" . _('Subscribe to:') . '<br>' . '<tt><input type="text" name="mailbox[]" size=35>' . '<INPUT TYPE=SUBMIT VALUE="' . _('Subscribe') . "\">\n" . "</FORM></TD></TR></TABLE><BR>\n";
}
