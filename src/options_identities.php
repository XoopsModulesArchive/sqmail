 ...
 *
 * $Id: options_identities.php,v 1.22 2003/03/03 05:44:34 ebullient Exp $
 */
if (!isset($xoopsIntro)) {
...
echo '<br>' . html_tag(
    ...
    "\n" . html_tag(
            ...
            "\n" . html_tag(
                'td',
                "\n" . '<b>' . _('Options') . ' - ' . _('Advanced Identities') . '</b><br>' . html_tag(
                    'table',
                    "\n" . html_tag(
                        'tr',
                        "\n" . html_tag(
...
function sti_input($title, $hd, $data, $post, $bg)
{
    $return_val = html_tag(
        'tr',
        html_tag('td', $title . ':', 'right', '', 'nowrap') . html_tag('td', '<input size="50" type="text" value="' . htmlspecialchars($data, ENT_QUOTES | ENT_HTML5) . '" name="' . $hd . $post . '">', 'left'),
        '',
        $bg
    );
...
function sti_textarea($title, $hd, $data, $post, $bg)
{
    $return_val = html_tag(
        'tr',
        html_tag('td', $title . ':', 'right', '', 'nowrap') . html_tag('td', '<textarea cols="50" rows="5" name="' . $hd . $post . '">' . htmlspecialchars($data, ENT_QUOTES | ENT_HTML5) . '</textarea>', 'left'),
        '',
        $bg
    );
