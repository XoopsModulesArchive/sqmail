 ...
 *
 * $Id: read_body.php,v 1.283 2003/03/09 18:54:34 ebullient Exp $
 */
if (!isset($xoopsIntro)) {
 ...
 *
 * @param mixed $passed_id
 * @return the index of the next valid message from the array
 */
function findNextMessage($passed_id)
{
    global $msort, $msgs, $sort, $thread_sort_messages, $allow_server_sort, $server_sort_array;
...
{
    global $msort, $sort, $msgs, $thread_sort_messages, $allow_server_sort, $server_sort_array;
...
function printer_friendly_link($mailbox, $passed_id, $passed_ent_id, $color)
{
    ...
    if ($javascript_on) {
        ...
        $result = '<a target="_blank" href="../src/printer_friendly_bottom.php' . "$params\">$print_text</a>\n";
    }
...
function SendMDN($mailbox, $passed_id, $sender, $message, $imapConnection)
{
    ...
    if (isset($identity) && 'default' != $identity) {
        $from_mail = getPref(
            $data_dir,
            $username,
    ...
    if (isset($languages[$squirrelmail_language]['XTRA_CODE'])
        ...
        if ('iso-2022-jp' == mb_strtolower($default_charset)) {
...
function formatEnvheader(
    ...
    if ($default_use_priority) {
        $env[_('Priority')] = htmlspecialchars(getPriorityStr($header->priority), ENT_QUOTES | ENT_HTML5);
    }
    ...
    echo '<TABLE BGCOLOR="' . $color[9] . '" WIDTH="100%" CELLPADDING="1"' . ' CELLSPACING="0" BORDER="0" ALIIGN="center">' . "\n";
...
{
    global $base_uri, $draft_folder, $where, $what, $color, $sort, $startMessage, $compose_new_win, $PHP_SELF, $save_as_draft, $enable_forward_as_attachment;
    ...
    if (isset($where) && isset($what)) {
        ...
        $msgs_url .= 'webmail.php?sort=' . $sort . '&amp;startMessage=' . $startMessage . '&amp;mailbox=' . $urlMailbox;
    ...
    $comp_uri = $base_uri . 'src/compose.php' . '?passed_id=' . $passed_id . '&amp;mailbox=' . $urlMailbox . (isset($passed_ent_id) ? '&amp;passed_ent_id=' . $passed_ent_id : '');
    ...
    if (!(isset($where) && isset($what)) && !$passed_ent_id) {
    ...
    } elseif (isset($passed_ent_id) && $passed_ent_id) {
        ...
        foreach ($message->parent->entities as $ent) {
            if ('message' == $ent->type0 && 'rfc822' == $ent->type1) {
                $c++;
        ...
        if ($par_ent_id) {
            $par_ent_id = mb_substr($par_ent_id, 0, -2);
...
function formatToolbar($mailbox, $passed_id, $passed_ent_id, $message, $color)
{
    ...
    $s = "<TR>\n" . html_tag('td', '', 'right', '', 'VALIGN="MIDDLE" WIDTH="20%"') . '<B>' . _('Options') . ":&nbsp;&nbsp;</B></TD>\n" . html_tag('td', '', 'left', '', 'VALIGN="MIDDLE" WIDTH="80%"') . '<SMALL>' . '<a href="' . $url . '">' . _('View Full Header') . '</a>';
...
if (isset($passed_ent_id) && $passed_ent_id) {
    ...
    if ('message' != $message->type0 && 'rfc822' != $message->type1) {
...
if (1 == $show_html_default) {
    ...
    $ent_ar = $message->findDisplayEntity([], ['text/plain']);
}
$cnt = count($ent_ar);
for ($i = 0; $i < $cnt; $i++) {
