 ...
 *
 * $Id: addrbook_search_html.php,v 1.58 2003/03/27 16:47:45 philippe_mingo Exp $
 */
if (!isset($xoopsIntro)) {
...
function addr_insert_hidden()
{
    ...
    if ("\r" == mb_substr($body, 0, 1)) {
        echo "\n";
    }

    echo htmlspecialchars($body, ENT_QUOTES | ENT_HTML5)
         . '" name=body>'
         . "\n"
         ...
         . "\n"
         . '<input type=hidden value="'
         . htmlspecialchars($send_to_cc, ENT_QUOTES | ENT_HTML5)
         . '" name=send_to_cc>'
         . "\n"
         . '<input type=hidden value="'
         . htmlspecialchars($send_to_bcc, ENT_QUOTES | ENT_HTML5)
         . '" name=send_to_bcc>'
         . "\n"
         . '<input type=hidden value="'
         . htmlspecialchars($identity, ENT_QUOTES | ENT_HTML5)
         . '" name=identity>'
         . "\n"
...
function addr_display_result($res, $includesource = true)
{
    ...
    if ($javascript_on) {
        ...
        $chk_all = '<a href="#" onClick="CheckAll(\'T\');">'
                   . _('All')
                   . '</a>&nbsp;<font color="'
                   . $color[9]
                   . '">To</font>'
    ...
    foreach ($res as $row) {
        ...
        if ('ja_JP' == $squirrelmail_language) {
            echo html_tag('tr', '', '', $tr_bgcolor, 'nowrap') . html_tag(
                'td',
                '<input type=checkbox name="send_to_search[T'
                    . $line
                    . ']" value = "'
                . htmlspecialchars($email, ENT_QUOTES | ENT_HTML5)
                    . '">&nbsp;'
                    . _('To')
                    . '&nbsp;'
                ...
                '',
                'width="5%" nowrap'
            ) . html_tag('td', '&nbsp;' . htmlspecialchars($row['lastname'], ENT_QUOTES | ENT_HTML5) . ' ' . htmlspecialchars($row['firstname'], ENT_QUOTES | ENT_HTML5) . '&nbsp;', 'left', '', 'nowrap') . html_tag('td', '&nbsp;' . htmlspecialchars($row['email'], ENT_QUOTES | ENT_HTML5) . '&nbsp;', 'left', '', 'nowrap') . html_tag(
                    'td',
                    '&nbsp;' . htmlspecialchars($row['label'], ENT_QUOTES | ENT_HTML5) . '&nbsp;',
                    'left',
                    '',
                    'nowrap'
                );
            ...
            echo html_tag('tr', '', '', $tr_bgcolor, 'nowrap') . html_tag(
                ...
                '',
                'width="5%" nowrap'
            ) . html_tag('td', '&nbsp;' . htmlspecialchars($row['name'], ENT_QUOTES | ENT_HTML5) . '&nbsp;', 'left', '', 'nowrap') . html_tag('td', '&nbsp;' . htmlspecialchars($row['email'], ENT_QUOTES | ENT_HTML5) . '&nbsp;', 'left', '', 'nowrap') . html_tag('td', '&nbsp;' . htmlspecialchars(
                                                                                                                                                                                                                                                                            $row['label'],
    ...
    echo html_tag(
        ...
        html_tag(
                 'td',
                 '<INPUT TYPE=submit NAME="addr_search_done" VALUE="' . _('Use Addresses') . '">',
                 'center',
                 '',
                 'colspan="' . $td_colspan . '"'
             )
    ) . '</TABLE>' . '<INPUT TYPE=hidden VALUE=1 NAME="html_addr_search_done">' . '</FORM>';
}
...
echo '<br>' . html_tag(
    ...
    'center',
    '',
    'width="95%" cellpadding="2" cellspacing="2" border="0"'
);
...
echo '  <INPUT TYPE=text NAME=addrquery VALUE="' . htmlspecialchars($addrquery, ENT_QUOTES | ENT_HTML5) . "\" SIZE=26>\n";
...
if ('' == $addrquery && empty($listall)) {
    if (!isset($backend) || -1 != $backend || '' == $addrquery) {
        if ('' == $addrquery) {
    ...
    if (!empty($addrquery)) {
        ...
        if (!is_array($res)) {
            echo html_tag(
                'p',
                '<b><br>' . _('Your search failed with the following error(s)') . ':<br>' . $abook->error . "</b>\n",
                'center'
            ) . "\n";
        } else {
            if (0 == count($res)) {
                echo html_tag(
                    'p',
                    '<br><b>' . _('No persons matching your search was found') . "</b>\n",
                    'center'
                ) . "\n";
            } else {
