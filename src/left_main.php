 ...
 *
 * $Id: left_main.php,v 1.154 2003/03/11 17:39:19 kink Exp $
 */
...
function formatMailboxName($imapConnection, $box_array)
{
    ...
    if ((2 == $unseen_notify && 'INBOX' == $real_box)
        || 3 == $unseen_notify) {
    ...
    if (false !== $status) {
        ...
        $line .= '<a href="webmail.php?PG_SHOWALL=0&amp;sort=0&amp;startMessage=1&amp;mailbox=' . $mailboxURL . '" STYLE="text-decoration:none">';
    }
    ...
    if (($move_to_trash) && ($real_box == $trash_folder)) {
        ...
        if (($numMessages > 0) or (1 == $box_array['parent'])) {
...
function compute_folder_children(&$parbox, $boxcount)
{
    ...
    while (($nextbox < $boxcount)
        ...
        $boxes[$nextbox]['visible'] = ($boxes[$parbox]['visible']
                                       && (SM_BOX_COLLAPSED != $boxes[$parbox]['collapse']));
...
function create_collapse_link($boxnum)
{
    ...
    /* Create the link for this collapse link. */ //    $link = '<a target="left" style="text-decoration:none" ' .

    //            'href="left_main.php?';

    $link = '<a style="text-decoration:none" ' . 'href="webmail.php?';

    if (SM_BOX_COLLAPSED == $boxes[$boxnum]['collapse']) {
        $link .= "unfold=$mailbox\">+";
    } else {
        $link .= "fold=$mailbox\">-";
    }
 ...
 * @param        current $imapConnection imap connection in use
 * @param mixed $unseen_type
...
function create_unseen_string($boxName, $boxArray, $imapConnection, $unseen_type)
{
    ...
    if (SM_BOX_COLLAPSED == $boxArray['collapse'] && $unseen_cum) {
        /* Collect the counts for this boxes subfolders. */

        $curBoxLength = mb_strlen($boxName);
        ...
        for ($i = 0; $i < $boxCount; ++$i) {
            ...
            if (($boxName != $boxes[$i]['unformatted'])
                && (mb_substr($boxes[$i]['unformatted'], 0, $curBoxLength) == $boxName)
                && !in_array('noselect', $boxes[$i]['flags'], true)) {
    ...
    if ((1 == $unseen_type) && ($boxUnseenCount > 0)) {
        $result[0] = "($boxUnseenCount)";
    } elseif (2 == $unseen_type) {
 ...
 * @param mixed $curbox_name
 * @param mixed $parbox_name
 * @return bool
 * @return bool
 */
function is_parent_box($curbox_name, $parbox_name)
{
    ...
    $actual_parname = mb_substr($actual_parname, 0, mb_strlen($parbox_name));
...
{
    global $data_dir, $username, $startmessage, $color, $unseen_notify, $unseen_type, $move_to_trash, $trash_folder, $collapse_folders;
    ...
    if ($boxes) {
        ...
        if (($move_to_trash) && ($mailbox == $trash_folder)) {
            ...
            if (!$boxes->is_noselect) {
                ...
                $pre .= "<a href=\"webmail.php?PG_SHOWALL=0&amp;sort=0&amp;startMessage=1&amp;mailbox=$mailboxURL\">";
        ...
        if (!$collapse || $boxes->is_root) {
            for ($i = 0, $iMax = count($boxes->mbxs); $i < $iMax; $i++) {
    ...
    if ($boxes) {
        ...
        if (($move_to_trash) && ($mailbox == $trash_folder)) {
            ...
            if (!$boxes->is_noselect) { /* \Noselect boxes can't be selected */
                ...
                $pre .= "<a class=\"mbx_link\" href=\"webmail.php?PG_SHOWALL=0&amp;sort=0&amp;startMessage=1&amp;mailbox=$mailboxURL\">";
        ...
        if (!$boxes->is_root) {
            if ($use_folder_images) {
                ...
                } elseif ($boxes->is_sent) {
                    $folder_img = '../images/senti.gif';
                } elseif ($boxes->is_trash) {
                    $folder_img = '../images/delitem.gif';
                } elseif ($boxes->is_draft) {
                    $folder_img = '../images/draft.gif';
                } else {
                    $folder_img = '../images/folder.gif';
                }
...
//             "<meta http-equiv=\"REFRESH\" content=\"$left_refresh;URL=webmail.php\">\n";
//} else {
//    $xtra = '';
//}
...
$xoops_module_header = displayHtmlHeader('SquirrelMail', $xtra);

/* If requested and not yet complete, attempt to autocreate folders. */
if ($auto_create_special && !isset($auto_create_done)) {
    $autocreate = [$sent_folder, $trash_folder, $draft_folder];
...
echo "<table bgcolor=\"$color[4]\" cellspacing=\"0\" border=\"0\"><tr bgcolor=\"$color[9]\"><td>";
...
echo "\n\n" . html_tag('table', '', 'left', '', 'border="0" cellspacing="0" cellpadding="0" width="99%"') . html_tag('tr') . html_tag('td', '', 'left') . '<center><font size="4"><b>' . _('Folders') . "</b><br></font>\n\n";
...
if (6 != $date_format) {
    ...
    switch ($date_format) {
        ...
        case 5:
            ...
            break;
        default:
            $clk = mb_substr(getDayName(date('w', time())), 0, 3) . date(', ' . $hr, time());
    }

    $clk = str_replace(' ', '&nbsp;', $clk);

    echo '<center><small>' . str_replace(' ', '&nbsp;', _('Last Refresh')) . ": $clk</small></center>";
}

/* Next, display the refresh button. */
//echo '<small>(<a href="../src/left_main.php" target="left">'.
echo '<small>(<a href="../src/webmail.php">' . _('refresh folder list') . '</a>)</small></center><br>';
...
if ($oldway) {  /* normal behaviour SM */
    ...
    for ($i = 0, $iMax = count($boxes); $i < $iMax; $i++) {
        if ($boxes[$i]['visible']) {
            $_mailbox = $boxes[$i]['formatted'];

            $mblevel = mb_substr_count($boxes[$i]['unformatted'], $delimiter) + 1;
            ...
            if (isset($collapse_folders) && $collapse_folders && $boxes[$i]['parent']) {
                $prefix = str_replace(' ', '&nbsp;', mb_substr($prefix, 0, -2)) . create_collapse_link($i) . '&nbsp;';
            } else {
            ...
            if (in_array('noselect', $boxes[$i]['flags'], true)) {
                ...
                if (preg_match('^( *)([^ ]*)', $mailbox, $regs)) {
                    $_mailbox = str_replace('&nbsp;', '', $_mailbox);

                    $line .= str_replace(' ', '&nbsp;', $_mailbox);
                }
